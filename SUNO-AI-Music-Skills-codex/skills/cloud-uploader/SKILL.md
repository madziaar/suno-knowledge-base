...
After generating promo videos with ``promo-director` skill`, upload them to cloud storage for:
- Hosting on websites
...
See `this skill package root/reference/cloud/setup-guide.md` for detailed setup instructions.

### Required Files

- Promo videos generated (run ``promo-director` skill` first)
...
~/.bitwize-music/venv/bin/pip install -r this skill package root/requirements.txt
...
Generate with: `promo-director` skill {album}
```
...
3. Continue to release: `release-director` skill {album}
```
...
**Basic upload:**
```
`cloud-uploader` skill my-album
```

**Preview only:**
```
`cloud-uploader` skill my-album --dry-run
```

**Upload promos only:**
```
`cloud-uploader` skill my-album --type promos
```

**Upload with public access:**
```
`cloud-uploader` skill my-album --public
```
...
- See `this skill package root/reference/cloud/setup-guide.md`
...
- Generate promo videos first: ``promo-director` skill {album}`
...
**Optional:** Upload to cloud storage: `cloud-uploader` skill {album}
```
...
Ready for release workflow: `release-director` skill {album}
```
...
- `this skill package root/reference/cloud/setup-guide.md` - Detailed setup instructions
- `this skill package root/skills/promo-director/SKILL.md` - Generate promo videos
- `this skill package root/skills/release-director/SKILL.md` - Release workflow

## Model Recommendation

**default model** - This skill runs scripts and coordinates workflow. No creative output from LLM.
