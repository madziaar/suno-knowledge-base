...
Call `get_python_command()` to verify the venv exists. If `venv_exists` is false, show the warning and suggest ``setup` skill`.
...
Options:
  1. Use `import-art` skill to place artwork
...
4. [Optional] Upload to cloud: `cloud-uploader` skill {album}
5. Ready for release workflow: `release-director` skill {album}
```
