...
from suno_utils import safe_messagebox
...
class VaultTab(ctk.CTkFrame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # --- Left Sidebar ---
        self.sidebar = ctk.CTkFrame(self, width=250, corner_radius=0, fg_color="#18181b")
        ...
        self.search = ctk.CTkEntry(self.sidebar, placeholder_text="Search prompts...", textvariable=self.search_var)
        self.search.pack(fill="x", padx=10, pady=10)
        
        # Add Button
        ctk.CTkButton(self.sidebar, text="+ New Prompt", command=self.new_prompt, fg_color="#7c3aed", hover_color="#6d28d9").pack(fill="x", padx=10, pady=(0, 10))
        ...
        # Title Input
        ctk.CTkLabel(self.content, text="Title", anchor="w", font=("Segoe UI", 12, "bold")).pack(anchor="w")
        self.title_entry = ctk.CTkEntry(self.content, placeholder_text="My Awesome Prompt")
        self.title_entry.pack(fill="x", pady=(5, 15))
        
        # Tags Input
        ctk.CTkLabel(self.content, text="Tags (comma separated)", anchor="w", font=("Segoe UI", 12, "bold")).pack(anchor="w")
        self.tags_entry = ctk.CTkEntry(self.content, placeholder_text="Dark, Techno, Fast")
        self.tags_entry.pack(fill="x", pady=(5, 15))
        
        # Prompt Text
        ctk.CTkLabel(self.content, text="Prompt", anchor="w", font=("Segoe UI", 12, "bold")).pack(anchor="w")
        ...
        self.save_btn = ctk.CTkButton(self.btn_frame, text="Save", command=self.save_prompt, width=100)
        self.save_btn.pack(side="right", padx=5)
        
        self.copy_btn = ctk.CTkButton(self.btn_frame, text="Copy to Clipboard", command=self.copy_to_clipboard, fg_color="#22c55e", hover_color="#16a34a")
        self.copy_btn.pack(side="right", padx=5)
        
        self.del_btn = ctk.CTkButton(self.btn_frame, text="Delete", command=self.delete_current, fg_color="#ef4444", hover_color="#dc2626", width=100)
        self.del_btn.pack(side="left", padx=5)
        
        self.status_lbl = ctk.CTkLabel(self.btn_frame, text="", text_color="#22c55e")
    ...
    def refresh_list(self, *args):
        ...
        for uid, data in all_prompts.items():
            ...
            btn = ctk.CTkButton(self.scroll_list, text=title, anchor="w", 
                                fg_color="transparent", text_color="gray90", hover_color="#3f3f46",
    ...
    def load_prompt(self, uid):
        ...
        for u, btn in self.prompt_buttons.items():
            btn.configure(fg_color="#3f3f46" if u == uid else "transparent")
