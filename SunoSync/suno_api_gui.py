...
class SunoApiApp:
    ...
    def create_widgets(self):
        ...
        self.root.columnconfigure(0, weight=0, minsize=400) # Left Column (Fixed/Min width)
        self.root.columnconfigure(1, weight=1)              # Right Column (Expands)
        self.root.rowconfigure(0, weight=1)                 # Full Height

        # --- LEFT COLUMN: Control Panel ---
        left_panel = tk.Frame(self.root, bg=self.bg_dark, padx=20, pady=20)
        ...
        right_panel = tk.Frame(self.root, bg=self.bg_dark, padx=20, pady=20)
    ...
    def _pulse_status(self):
        ...
        self._status_pulse_job = self.root.after(600, self._pulse_status)
    ...
    def open_filters(self):
        ...
        FilterPopup(self.root, self.filter_settings, self.on_filters_applied, active_workspace_name=ws_name,
    ...
    def _fetch_thumb_bg(self, uuid, url):
        ...
        if data:
            self.root.after(0, lambda: self.queue_pane.update_thumbnail(uuid, data))
    ...
    def _fetch_workspaces_thread(self, token):
        ...
        self.root.after(0, lambda: self._show_workspace_browser(workspaces))
    ...
    def _show_workspace_browser(self, workspaces):
        ...
        if not workspaces:
            ...
            return
        WorkspaceBrowser(self.root, workspaces, self.on_workspace_selected,
    ...
    def on_preload_complete_safe(self, success):
        self.root.after(0, lambda: self.on_preload_complete(success))
