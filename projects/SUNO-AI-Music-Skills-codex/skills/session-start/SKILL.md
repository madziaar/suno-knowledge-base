...
- If config missing (`~/.bitwize-music/config.yaml` doesn't exist): suggest ``configure` skill`
...
- `status: "no_venv"` → **stop** and suggest ``setup` skill`
...
If missing, tell user to run ``configure` skill`.
...
Compare `plugin_version` in state.json against current version in `this skill package root/VERSION`:
...
3. **If stored < current** (upgrade detected):
   - Read migration files from `this skill package root/migrations/` for versions between stored and current
...
Skill model checking is no longer part of session start. Run ``skill-model-updater` skill check` manually when new Codex models are released.
...
| Condition | Tip |
|-----------|-----|
| No albums exist | "Try ``tutorial` skill` to create your first album" |
| Ideas exist but no albums | "You have album ideas! Use ``album-ideas` skill list` to review them" |
| In-progress albums exist | "Resume where you left off: ``resume` skill <album-name>`" |
...
- "Use ``resume` skill` to quickly jump back into an album"
...
- "Use ``clipboard` skill` to copy lyrics/prompts for Suno"
- "Master your audio with ``mastering-engineer` skill` for professional results"
