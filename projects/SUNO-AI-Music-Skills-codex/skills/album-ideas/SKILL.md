...
Backlog of album concepts. When ready to start working on an idea, run ``new-album` skill` to create the album directory and move the idea to "In Progress".
...
To start working on this album:
  `new-album` skill "Album Title" [genre]
```
...
- If NOT found, suggest: "Run ``new-album` skill` to create the album structure"
...
CLAUDE.md already mentions checking IDEAS.md at session start. When Codex checks ideas:
...
1. Run ``new-album` skill [title] [genre]`
