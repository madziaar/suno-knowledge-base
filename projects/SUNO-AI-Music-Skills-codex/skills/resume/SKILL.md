...
**Usage**:
```
`resume` skill <album-name>
`resume` skill my-album
`resume` skill "demo album"
```
...
2. If not found: MCP returns available albums — suggest closest match or ``new-album` skill`
...
**Instrumental detection**: Check each track's frontmatter for `instrumental: true` or Track Details table for `**Instrumental** | Yes`. Instrumental tracks skip the lyrics workflow entirely and go straight to ``suno-engineer` skill`.

```
Album Status = "Concept"
  → "Define the album concept. Run `album-conceptualizer` skill"

Album Status = "Research Complete"
  → Any tracks Sources Pending?
    YES → "Sources need verification. Run `verify-sources` skill [album]"
...
Album has tracks with "Not Started"
  → Is the first not-started track instrumental?
    YES → "Create Style Box for [track]. Use `suno-engineer` skill directly (instrumental track)"
    NO  → "Write lyrics for [first not-started track]. Use `lyric-writer` skill"

Album has tracks with "In Progress" (lyrics partially written)
  → "Finish lyrics for [first in-progress track]. Use `lyric-writer` skill"

Album has tracks with "Sources Pending"
  → "Verify sources for [track]. Run `verify-sources` skill [album]"

All tracks have lyrics (or Style Box for instrumentals), none generated
  → Mixed album (vocal + instrumental)?
    YES → "All tracks ready! Run `pronunciation-specialist` skill on vocal tracks, then `lyric-reviewer` skill, then `pre-generation-check` skill to validate all gates (instrumental tracks auto-skip lyrics gates)."
    NO  → "All lyrics complete! Style prompts should be ready. Run `pronunciation-specialist` skill to check for pronunciation risks, then `lyric-reviewer` skill for final QC, then `pre-generation-check` skill to validate all gates before generating on Suno."
  ...
  → Any Generated tracks without ✓ in Generation Log Rating?
    YES → "Track [name] was generated but not approved. Listen and decide:
           ...
           - Not happy? Log the reason, then:
             Style issue → `suno-engineer` skill to revise Style Box
             Lyrics issue → `lyric-writer` skill to fix, then regenerate
             Bad luck → Regenerate on Suno with same settings (it's non-deterministic)"
    NO  → "Generate [first un-generated track] on Suno. Use `suno-engineer` skill"
...
Album Status = "Released"
  → "This album is released! Consider `promo-director` skill for promotional content"
  → Also suggest: "Start a new album? Check `album-ideas` skill list"
```
...
4. If no albums exist, suggest ``new-album` skill`
...
Did you mean one of these? Or use `new-album` skill to create a new album.
```
...
Use **default model** - This is a coordination/reporting task, not creative work.
