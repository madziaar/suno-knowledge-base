...
def make_vocal(
    duration: float = 2.0, rate: int = DEFAULT_RATE
...
def make_full_mix(
    duration: float = 3.0, rate: int = DEFAULT_RATE
) -> tuple[np.ndarray, int]:
...
def make_phase_partial(
    ...
    left = (
        0.5 * np.sin(2 * np.pi * 440 * t) + 0.3 * np.sin(2 * np.pi * 660 * t)
    ).astype(np.float64)
    # Right: same tones but shifted 90 degrees (cosine instead of sine)
    right = (
        0.5 * np.cos(2 * np.pi * 440 * t) + 0.3 * np.cos(2 * np.pi * 660 * t)
    ).astype(np.float64)
