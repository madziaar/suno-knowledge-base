...
class TestSkillCount:
    ...
    def test_readme_skill_count(self, project_root, all_skill_frontmatter):
        ...
        match = re.search(
            r"\*\*(\d+)\s+specialized skills\*\*", readme_content
        ...
        assert claimed == actual, f"README claims {claimed} skills, actual is {actual}"
...
class TestVersionSync:
    ...
    def test_version_files_match(self, project_root):
        ...
        with open(marketplace_json) as f:
            ...
            marketplace_version = marketplace_data.get("plugins", [{}])[0].get(
                "version", "unknown"
            )
