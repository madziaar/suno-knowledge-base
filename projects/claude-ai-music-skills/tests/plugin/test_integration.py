...
class TestPrerequisites:
    ...
    def test_valid_prerequisites(self, all_skill_frontmatter):
        ...
        assert not invalid, "Invalid prerequisite references:\n" + "\n".join(invalid)
...
class TestLyricWorkflowChain:
    ...
    def test_reviewer_covers_writer_checklist(self, all_skill_frontmatter):
        ...
        writer_match = re.search(
            r"(?:(\d+)-[Pp]oint.*(?:[Cc]hecklist|[Qq]uality [Cc]heck)|[Qq]uality [Cc]heck \((\d+)-[Pp]oint\))",
            writer_content,
        )
        reviewer_match = re.search(r"(\d+)-[Pp]oint.*[Cc]hecklist", reviewer_content)
...
class TestInstrumentalRouting:
    ...
    def test_instrumental_routes_to_suno_engineer(
        self, all_skill_frontmatter, skill_name
    ):
