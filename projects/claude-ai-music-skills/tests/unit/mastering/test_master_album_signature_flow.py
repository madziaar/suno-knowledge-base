...
def _install_album(
    monkeypatch, audio_path: Path, album_slug: str, status: str = "In Progress"
) -> None:
...
def test_master_album_does_not_write_signature_on_stage_failure(
...
def test_master_album_signature_write_failure_is_nonfatal(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    with patch.object(
        processing_helpers, "_resolve_audio_dir", _fake_resolve
...
def test_released_album_with_signature_enters_frozen_mode(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_in_progress_album_uses_fresh_mode(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result["stages"]["anchor_selection"]["method"] in (
        "composite",
        "tie_breaker",
    )
...
def test_frozen_mode_preserves_original_anchor_block(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_frozen_mode_delivery_matches_frozen_target(
    tmp_path: Path, monkeypatch
) -> None:
