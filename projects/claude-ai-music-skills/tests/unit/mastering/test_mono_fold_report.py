...
def test_includes_track_name_in_header():
    md = render_mono_fold_markdown(
        "01-test-track", _pass_metrics(), sample_filename="01-test-track.mono.wav"
    )
...
def test_pass_verdict_rendered():
    md = render_mono_fold_markdown(
        "track", _pass_metrics(), sample_filename="track.mono.wav"
    )
    assert "PASS" in md
...
def test_fail_verdict_rendered():
    md = render_mono_fold_markdown(
        "track", _fail_metrics(), sample_filename="track.mono.wav"
    )
    assert "FAIL" in md
