...
def test_album_coherence_check_falls_back_to_defaults_when_genre_empty_with_anchor(
    tmp_path: Path,
) -> None:
...
def test_album_coherence_correct_dry_run_does_not_write(tmp_path: Path) -> None:
    ...
    assert (
        tmp_path / "mastered" / "02-track.wav"
    ).read_bytes() == mastered_bytes_before
...
def test_album_coherence_correct_errors_when_polished_missing(tmp_path: Path) -> None:
    ...
    assert "error" in result
    assert (
        "polished" in result["error"].lower() or "not found" in result["error"].lower()
    )


def test_album_coherence_correct_errors_when_polished_missing_a_track(
    tmp_path: Path,
) -> None:
...
def test_album_coherence_correct_remasters_lufs_outlier(tmp_path: Path) -> None:
    ...
    correction = next(
        c for c in result["corrections"] if c["filename"] == "02-track.wav"
    )
