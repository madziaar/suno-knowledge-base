...
class TestGenrePresets:
    ...
    def test_preset_with_mastering(self, noise_wav, output_path):
        ...
        if preset["cut_highs"] != 0:
            eq.append(
                (preset["eq_highs_freq"], preset["cut_highs"], preset["eq_highs_q"])
            )
        result = master_track(
            noise_wav, output_path, target_lufs=preset["target_lufs"], eq_settings=eq
        )
        assert not result.get("skipped", False)
...
class TestLookAheadLimiter:
    ...
    def test_zero_lookahead_falls_back(self):
        ...
        result_zero = limit_peaks_lookahead(
            data.copy(), ceiling_db=-3.0, lookahead_ms=0, rate=rate
        )
    ...
    def test_different_from_reactive(self):
        ...
        lookahead = limit_peaks_lookahead(
            data.copy(), ceiling_db=-3.0, lookahead_ms=5.0, rate=rate
        )
...
class TestLinearPhaseEQ:
    ...
    def test_apply_peaking_changes_signal(self):
        ...
        result = apply_linear_phase_eq(
            data, rate, 1000.0, -6.0, q=1.0, filter_type="peaking"
        )
