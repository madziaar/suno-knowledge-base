...
def test_master_album_no_upsampling_notice_when_rates_match(
    ...
    with patch.object(
        processing_helpers, "_resolve_audio_dir", _fake_resolve
    ), patch.object(shared_mod, "cache", _MockCache()), patch.object(
        mastering_config_mod, "load_mastering_config", return_value=custom
    ):
        result_json = asyncio.run(audio_mod.master_album("test-album"))
...
def two_track_long_audio_dir(tmp_path: Path) -> Path:
    ...
    sf.write(
        str(audio_dir / "01-track.wav"),
        np.column_stack([tone1, tone1]),
        sr,
        subtype="PCM_16",
    )
    ...
    sf.write(
        str(audio_dir / "02-track.wav"),
        np.column_stack([tone2, tone2]),
        sr,
        subtype="PCM_16",
    )
    return audio_dir
...
def test_master_album_records_anchor_selection_stage(
    ...
    with patch.object(
        ...
        result_json = asyncio.run(audio_mod.master_album("test-album", genre="pop"))
