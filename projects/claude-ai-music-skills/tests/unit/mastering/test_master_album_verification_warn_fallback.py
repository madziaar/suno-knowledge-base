...
def _make_ctx(
    tmp_path: Path, target_lufs: float = -14.0, ceiling: float = -1.5
) -> MasterAlbumCtx:
...
def test_verification_warn_fallback_on_unrecoverable(tmp_path: Path) -> None:
    ...
    fake_lufs = _fake_analyze(
        ctx.effective_lufs, off_by_lufs=-9.0, peak_db=ctx.effective_ceiling - 0.05
    )
    ...
    warnings_blob = " ".join(w if isinstance(w, str) else str(w) for w in ctx.warnings)
    assert "warn-fallback" in notices_blob.lower()
    assert (
        "unrecoverable" in warnings_blob.lower()
        or "VERIFICATION_WARNINGS" in warnings_blob
    )
...
def test_verification_halts_when_recovery_not_attempted(tmp_path: Path) -> None:
    ...
    fake_lufs = _fake_analyze(
        ctx.effective_lufs, off_by_lufs=0.0, peak_db=ctx.effective_ceiling + 0.5
    )
...
def test_verification_halts_on_mixed_failure(tmp_path: Path) -> None:
    ...
    ctx.output_dir = output_dir
    ctx.wav_files = [
        source_dir / "01-a-unrecoverable.wav",
        source_dir / "02-b-peak-issue.wav",
    ]
    ctx.mastered_files = [
        output_dir / "01-a-unrecoverable.wav",
...
def test_verification_halts_on_pure_range_failure(tmp_path: Path) -> None:
    ...
    asyncio.set_event_loop(loop)
    try:
        with patch("tools.mastering.analyze_tracks.analyze_track", _range_analyze):
            result = loop.run_until_complete(
                _album_stages._stage_verification(ctx),
            )
    finally:
