...
def test_measure_album_signature_no_anchor_returns_tracks_and_album(
    tmp_path: Path,
) -> None:
...
def test_measure_album_signature_missing_subfolder_returns_error(
    tmp_path: Path,
) -> None:
    def _fake_resolve(slug: str, *_: object, **__: object) -> tuple[str | None, Path]:
        return None, tmp_path
...
def test_measure_album_signature_with_genre_returns_anchor_block(
    tmp_path: Path,
) -> None:
    _setup_album(tmp_path, track_count=3)
...
def test_measure_album_signature_out_of_range_override_falls_through(
    tmp_path: Path,
) -> None:
    _setup_album(tmp_path, track_count=3)
