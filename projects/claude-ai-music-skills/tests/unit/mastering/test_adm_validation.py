...
def _write_sine(
    path: Path, *, amplitude: float = 0.3, duration: float = 1.0, rate: int = 44100
) -> Path:
...
def test_render_adm_validation_markdown_all_pass() -> None:
    ...
    md = render_adm_validation_markdown(
        "my-album", results, encoder_used="aac", ceiling_db=-1.0
    )
...
def test_check_aac_intersample_clips_silent_audio(tmp_path: Path) -> None:
    ...
    sf.write(
        str(silent_wav), np.zeros((44100, 2), dtype=np.float32), 44100, subtype="PCM_16"
    )
