...
class TestResolvePath:
    ...
    def test_content_path(self):
        ...
        expected = (
            Path("/tmp/test-content").resolve()
            / "artists/test-artist/albums/rock/my-album"
        )
        assert result == expected

    def test_audio_path(self):
        result = resolve_path(
            "audio", "my-album", genre="rock", config=self.SAMPLE_CONFIG
        )
        expected = (
            Path("/tmp/test-audio").resolve()
            / "artists/test-artist/albums/rock/my-album"
        )
        assert result == expected
    ...
    def test_documents_path(self):
        result = resolve_path(
            "documents", "my-album", genre="rock", config=self.SAMPLE_CONFIG
        )
        expected = (
            Path("/tmp/test-docs").resolve()
            / "artists/test-artist/albums/rock/my-album"
        )
        assert result == expected
    ...
    def test_artist_override(self):
        ...
        expected = (
            Path("/tmp/test-audio").resolve()
            / "artists/other-artist/albums/rock/my-album"
        )
        assert result == expected
...
class TestResolveTracksDir:
    ...
    def test_tracks_dir(self):
        ...
        expected = (
            Path("/tmp/test-content").resolve()
            / "artists/test-artist/albums/rock/my-album/tracks"
        )
        assert result == expected
