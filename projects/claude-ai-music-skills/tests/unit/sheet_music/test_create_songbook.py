...
class TestSanitizeFilename:
    ...
    def test_preserves_parentheses_and_hyphens(self):
        assert (
            sanitize_filename("Track (Remix) - Extended") == "Track (Remix) - Extended"
        )
...
class TestManifest:
    ...
    def test_manifest_with_filename_field(self, tmp_path):
        ...
        manifest = {
            "tracks": [
                {
                    "number": 1,
                    "source_slug": "01-apple-pie",
                    "title": "Apple Pie",
                    "filename": "01 - Apple Pie",
                },
                {
                    "number": 2,
                    "source_slug": "02-zebra-song",
                    "title": "Zebra Song",
    ...
    def test_fallback_without_manifest(self, tmp_path):
        ...
        numbered = sorted(
            [f for f in singles.glob("*.pdf") if re.match(r"^\d+", f.stem)]
        )
