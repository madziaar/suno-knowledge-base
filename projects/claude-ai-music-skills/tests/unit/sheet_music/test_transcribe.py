        ...
        with patch.object(Path, "exists", return_value=False):
            with patch(
                ...
                return_value=MagicMock(
                    returncode=0, stdout="/usr/local/bin/anthemscore\n"
                ),
            ):
                result = mod.find_anthemscore()
...
class TestTranscribeTrack:
    ...
    def test_successful_transcription(self, mock_run, wav_file, tmp_path, base_args):
        ...
        result = mod.transcribe_track(
            "/usr/bin/anthemscore", wav_file, tmp_path, base_args
        )
    ...
    @patch(
        "subprocess.run",
        side_effect=real_subprocess.TimeoutExpired(cmd="anthemscore", timeout=300),
    )
