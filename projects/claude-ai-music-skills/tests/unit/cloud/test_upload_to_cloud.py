...
class TestUploadFile:
    ...
    def test_dry_run_skips_upload(self, fake_video):
        ...
        result = mod.upload_file(
            client, "bucket", fake_video, "key/promo.mp4", dry_run=True
        )
        assert result is True
    ...
    def test_private_no_acl(self, fake_video):
        client = MagicMock()
        mod.upload_file(
            client, "bucket", fake_video, "key/promo.mp4", public_read=False
        )
        _, kwargs = client.upload_file.call_args
...
class TestRetryUpload:
    ...
    def test_success_first_attempt(self, mock_sleep, fake_video):
        client = MagicMock()
        result = mod.retry_upload(
            client, "bucket", fake_video, "key/f.mp4", max_retries=3
        )
        assert result is True
...
class TestFindAlbumPath:
    ...
    def test_mirrored_structure(self, tmp_path):
        album_dir = (
            tmp_path / "artists" / "testartist" / "albums" / "hip-hop" / "my-album"
        )
    ...
    def test_audio_root_override(self, tmp_path):
        ...
        result = mod.find_album_path(
            config, "my-album", audio_root_override=str(override_root)
        )
        assert result == album_dir
