...
def test_non_eq_analyzer_rec_ignored():
    ...
    assert "click_removal" not in merged or merged.get("click_removal") == baseline.get(
        "click_removal"
    )
...
class TestMixTrackStemsAnalyzerRecs:
    ...
    def test_mix_track_stems_records_overrides_applied_when_recs_present(
        self, tmp_path
    ):
    ...
    def test_mix_track_stems_missing_stem_in_recs_falls_through(self, tmp_path):
        ...
        analyzer_recs = {
            "synth": {
                "recommendations": {"high_tame_db": -2.5},
