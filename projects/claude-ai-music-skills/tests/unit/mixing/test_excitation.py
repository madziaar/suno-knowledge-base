...
def _pink_stereo(
    seconds: float = 2.0, rate: int = 48000, rms_db: float = -18.0
) -> np.ndarray:
...
def _dark_stereo(
    seconds: float = 2.0, rate: int = 48000, rms_db: float = -20.0
) -> np.ndarray:
...
def _high_mid_energy_pct(
    data: np.ndarray, rate: int, band: tuple[float, float] = (2000.0, 6000.0)
) -> float:
...
class TestHarmonicExcitation:
    ...
    def test_monotonic_in_amount(self):
        ...
        post_small = _high_mid_energy_pct(
            apply_harmonic_excitation(data, rate, amount_db=1.0),
            rate,
        )
        post_large = _high_mid_energy_pct(
            apply_harmonic_excitation(data, rate, amount_db=6.0),
            rate,
        )
