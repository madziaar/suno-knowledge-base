...
def test_harsh_condition_still_fires_above_0_25():
    ...
    t = np.linspace(0.0, 2.0, 2 * rate, endpoint=False)
    mono = (
        0.3 * np.sin(2 * np.pi * 3000 * t) + 0.3 * np.sin(2 * np.pi * 4000 * t)
    ).astype("float64")
    data = np.column_stack([mono, mono])
...
def test_middle_band_triggers_neither_condition():
    ...
    mono = (
        0.8 * np.sin(2 * np.pi * 500 * t) + 0.3 * np.sin(2 * np.pi * 3000 * t)
    ).astype("float64")
    data = np.column_stack([mono, mono])

    analyze_one = _build_analyzer(dark_ratio=0.10, harsh_ratio=0.25)
    result = analyze_one(
        data, rate, filename="middle-track.wav", stem_name="synth", genre="electronic"
    )
...
def test_preset_override_of_dark_threshold_changes_trigger():
    ...
    mono = (
        0.75 * np.sin(2 * np.pi * 500 * t) + 0.3 * np.sin(2 * np.pi * 3000 * t)
    ).astype("float64")
    data = np.column_stack([mono, mono])

    analyze_default = _build_analyzer(dark_ratio=0.10, harsh_ratio=0.25)
    result_default = analyze_default(
        data, rate, filename="mid.wav", stem_name="synth", genre="electronic"
    )
