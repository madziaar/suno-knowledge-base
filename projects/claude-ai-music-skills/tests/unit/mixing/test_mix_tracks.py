...
def stem_dir_6(tmp_path):
    ...
    _write_wav(
        stems / "1 Backing Vocals.wav", np.column_stack([backing, backing]), rate
    )
...
class TestMixTrackStemsMultiFile:
    ...
    def test_suno_naming_end_to_end(self, tmp_path):
        ...
        total = sum(len(v) if isinstance(v, list) else 1 for v in stem_paths.values())
        assert total == 4
...
class TestOverrideMerging:
    ...
    def test_override_merges_genre(self, tmp_path, monkeypatch):
        ...
        override_file.write_text("genres:\n  rock:\n    vocals:\n      gain_db: 2.0\n")
