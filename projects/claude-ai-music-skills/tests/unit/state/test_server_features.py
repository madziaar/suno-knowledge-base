...
class TestInstrumentalGateSkipping:
    ...
    def test_all_gates_run_for_non_instrumental_track(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "03"))
            )
    ...
    def test_instrumental_explicit_flag_gate(self, tmp_path):
        ...
        mock_cache, _ = self._make_cache_with_track(
            tmp_path, _INSTRUMENTAL_TRACK, sources_verified="N/A", explicit=False
        )
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _text_analysis_mod, "_artist_blocklist_cache", None
        ):
...
class TestAlbumAutoAdvancement:
    ...
    def _make_cache_with_album(
        self, tmp_path, album_status, track_statuses, has_sources_md=False
    ):
    ...
    def test_album_transition_validation_direct(self):
        ...
        # Valid transitions
        assert (
            _status_mod._validate_album_transition("Concept", "Research Complete")
            is None
        )
        assert _status_mod._validate_album_transition("Concept", "In Progress") is None
        assert (
            _status_mod._validate_album_transition(
                "Research Complete", "Sources Verified"
            )
            is None
        )
        assert (
            _status_mod._validate_album_transition("Sources Verified", "In Progress")
            is None
        )
        ...
        assert (
            _status_mod._validate_album_transition("In Progress", "Concept") is not None
        )
    ...
    def test_consistency_check_passes_for_early_statuses(self):
        ...
        assert (
            _status_mod._check_album_track_consistency(album, "Research Complete")
            is None
        )
        assert (
            _status_mod._check_album_track_consistency(album, "Sources Verified")
            is None
        )
