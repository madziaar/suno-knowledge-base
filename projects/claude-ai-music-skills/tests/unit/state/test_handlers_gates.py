...
"""
    + "\n".join(
        f"Line number {i} with some extra words to pad it out more" for i in range(120)
    )
    + """
```
...
class TestRunPreGenerationGates:
    ...
    def test_single_track_ready(self):
        ...
        with patch(
            "pathlib.Path.read_text", return_value=TRACK_FILE_COMPLETE
    ...
    def test_all_tracks_mixed_verdict(self):
        ...
        with patch(
            ...
            result = json.loads(_run(_gates_mod.run_pre_generation_gates("test-album")))
        assert result["found"] is True
    ...
    def test_track_not_found(self):
        result = json.loads(
            _run(_gates_mod.run_pre_generation_gates("test-album", "99-nonexistent"))
        )
        assert result["found"] is False
...
class TestCheckStreamingLyrics:
    ...
    def test_ready_track_passes_all(self):
        with patch("pathlib.Path.read_text", return_value=STREAMING_READY_FILE):
            result = json.loads(
                _run(_gates_mod.check_streaming_lyrics("test-album", "01-first-track"))
            )
    ...
    def test_uncapitalized_warned(self):
        ...
        track = result["tracks"][0]
        cap_check = next(
            c for c in track["checks"] if c["check"] == "Lines Capitalized"
        )
    ...
    def test_end_punctuation_warned(self):
        ...
        track = result["tracks"][0]
        punct_check = next(
            c for c in track["checks"] if c["check"] == "No End Punctuation"
        )
    ...
    def test_placeholder_blocked(self):
        ...
        placeholder_check = next(
            c for c in track["checks"] if c["check"] == "Not Placeholder"
        )
    ...
    def test_missing_section_blocked(self):
        ...
        track = result["tracks"][0]
        section_check = next(
            c for c in track["checks"] if c["check"] == "Section Exists"
        )
    ...
    def test_all_tracks_checked_when_no_slug(self):
        with patch("pathlib.Path.read_text", return_value=STREAMING_READY_FILE):
            result = json.loads(_run(_gates_mod.check_streaming_lyrics("test-album")))
        assert result["total_tracks"] == 2
    ...
    def test_album_not_found(self):
        result = json.loads(
            _run(_gates_mod.check_streaming_lyrics("nonexistent-album"))
        )
        assert result["found"] is False

    def test_track_not_found(self):
        result = json.loads(
            _run(_gates_mod.check_streaming_lyrics("test-album", "99-missing"))
        )
        assert result["found"] is False
