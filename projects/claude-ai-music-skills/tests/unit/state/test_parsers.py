...
class TestAlbumReadmeEdgeCases:
    ...
    def test_genre_from_path_with_albums_dir(self, tmp_path):
        ...
        result = _extract_genre_from_path(
            tmp_path / "artists" / "test" / "albums" / "rock" / "my-album" / "README.md"
        )
        assert result == "rock"
...
""")
        ...
        assert (
            result["description"]
            == "Writes or reviews lyrics with professional prosody."
        )
...
class TestParseAlbumReadmeMastering:
    ...
    def test_parse_album_readme_mastering_absent_is_empty_dict(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_parse_album_readme_mastering_malformed_is_empty_dict(
        self, tmp_path: Path
    ) -> None:
