...
class TestGetAlbumFull:
    ...
    def test_summary_only(self):
        result = json.loads(
            _run(_album_ops_mod.get_album_full("test-album", summary_only=True))
        )
        assert result["found"] is True
...
"""
        with patch("pathlib.Path.read_text", return_value=track_content):
            result = json.loads(
                _run(
                    _album_ops_mod.get_album_full(
                        "test-album", include_sections="lyrics,style"
                    )
                )
            )
        assert result["found"] is True
...
class TestValidateAlbumStructure:
    ...
    def test_album_not_found(self):
        result = json.loads(
            _run(_album_ops_mod.validate_album_structure("nonexistent"))
        )
        assert result["found"] is False
    ...
    def test_no_config_returns_error(self):
        ...
        result = json.loads(_run(_album_ops_mod.validate_album_structure("test-album")))
    ...
    def test_structure_checks_with_existing_dirs(self):
        ...
        with patch("pathlib.Path.is_dir", return_value=True), patch(
            ...
            result = json.loads(
                _run(
                    _album_ops_mod.validate_album_structure(
...
class TestCreateAlbumStructure:
    ...
    def test_genre_alias_resolved(self):
        ...
        with patch("pathlib.Path.exists", return_value=False), patch(
            "pathlib.Path.mkdir"
        ), patch("shutil.copy2"), patch("pathlib.Path.is_dir", return_value=False):
            result = json.loads(
                _run(_album_ops_mod.create_album_structure("new-album", "r&b"))
            )
    ...
    def test_successful_creation(self):
        ...
        with patch("pathlib.Path.exists", path_exists), patch(
            "pathlib.Path.mkdir"
        ), patch("shutil.copy2"), patch("pathlib.Path.is_dir", return_value=False):
            result = json.loads(
                _run(_album_ops_mod.create_album_structure("new-album", "rock"))
    ...
    def test_documentary_includes_research_files(self):
        ...
        album_path = Path(
            "/tmp/test-content/artists/test-artist/albums/hip-hop/doc-album"
        )
        ...
        with patch("pathlib.Path.exists", path_exists), patch(
            ...
            result = json.loads(
                _run(
                    _album_ops_mod.create_album_structure(
    ...
    def test_additional_genres_from_config(self):
        ...
        with patch("pathlib.Path.exists", return_value=False), patch(
            ...
            result = json.loads(
                _run(_album_ops_mod.create_album_structure("new-album", "synthwave"))
            )
        assert result["created"] is True
