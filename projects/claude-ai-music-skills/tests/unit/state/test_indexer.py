...
def _make_album_tree(
    content_root, artist, genre, album_slug, readme_text=None, tracks=None
):
...
def _override_indexer_paths(monkeypatch, cache_dir, config_path=None, lock_file=None):
    ...
    def test_track_missing_required_fields(self):
        ...
        state["albums"] = {
            "test-album": {
                ...
                "title": "Test",
                "status": "Concept",
                "tracks": {"01-track": {"path": "/tmp/track"}},
            }
        }
        errors = validate_state(state)
        ...
        with open(lock_file, "w") as fd:
            ...
            with patch("tools.state.indexer.fcntl.flock", side_effect=mock_flock):
                with patch("tools.state.indexer.time.sleep"):
                    with pytest.raises(
                        TimeoutError, match="Could not acquire state lock"
                    ):
...
class TestScanAlbums:
    ...
    def test_albums_with_tracks(self, tmp_path):
        ...
        _make_album_tree(
            ...
            "my-album",
            tracks={
                "01-first.md": _make_track_content(
                    "First", "Final", "[Listen](https://suno.com/song/abc)", "No", "N/A"
                ),
    ...
    def test_skip_album_with_parse_error(self, tmp_path):
        content_root = tmp_path / "content"
        album_dir = (
            content_root / "artists" / "testartist" / "albums" / "rock" / "bad-album"
        )
    ...
    def test_album_with_fixtures(self, tmp_path):
        ...
        shutil.copy(
            FIXTURES_DIR / "track-not-started.md", tracks_dir / "04-kernel-panic.md"
        )
...
"""
        _make_album_tree(
            content_root, "testartist", "rock", "anchor-album", readme_text=readme_text
        )
...
class TestIncrementalUpdate:
    ...
    def test_config_unchanged(self, tmp_path, monkeypatch):
        ...
        assert (
            updated["albums"]["my-album"]["tracks"]["01-track"]["status"]
            == "Not Started"
        )
    ...
    def test_album_removed(self, tmp_path, monkeypatch):
        ...
        album_to_remove = _make_album_tree(
            content_root, "testartist", "rock", "remove-album"
        )
    ...
    def test_track_updated(self, tmp_path, monkeypatch):
        ...
        existing["config"]["config_mtime"] = 100.0
        assert (
            existing["albums"]["my-album"]["tracks"]["01-track"]["status"]
            == "Not Started"
        )
...
"""
        _make_album_tree(
            content_root, "testartist", "rock", "my-album", readme_text=readme_initial
        )
...
class TestUpdateTracksIncremental:
    ...
    def test_completed_count_recomputed(self, tmp_path):
        ...
        (tracks_dir / "01-track.md").write_text(_make_track_content("One", "Final"))
        (tracks_dir / "02-track.md").write_text(_make_track_content("Two", "Generated"))
...
class TestValidateStateSkills:
    ...
    def test_skill_item_missing_required_fields(self):
        state = _make_minimal_state()
        state["skills"]["items"] = {"broken": {"name": "broken"}}
        errors = validate_state(state)
...
class TestBuildConfigSectionOverrides:
    ...
    def test_explicit_overrides_path(self, monkeypatch):
        ...
        section = build_config_section(config)
        assert section["overrides_dir"] == str(
            Path("/home/user/custom-overrides").resolve()
        )
    ...
    def test_empty_overrides_defaults_to_content_root(self, monkeypatch):
        ...
        section = build_config_section(config)
        assert section["overrides_dir"] == str(
            Path("/home/user/content").resolve() / "overrides"
        )
...
class TestIncrementalUpdateSkillsMtime:
    ...
    def test_skills_mtime_change_triggers_rescan(self, tmp_path, monkeypatch):
        ...
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text(
            _make_skill_content(name="test-skill", description="A test skill.")
        )
        ...
        (skill_dir2 / "SKILL.md").write_text(
            _make_skill_content(name="second-skill", description="Another skill.")
        )
