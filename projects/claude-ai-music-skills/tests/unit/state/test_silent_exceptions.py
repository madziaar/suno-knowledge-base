...
def _import_server():
    spec = importlib.util.spec_from_file_location(
        "state_server_silent_exc", SERVER_PATH
    )
...
class TestRenameReParseWarning:
    ...
    def test_reparse_failure_logs_warning(self, tmp_path, caplog):
        ...
        with (
            patch.object(
                rename_mod._shared.cache, "get_state", return_value=fake_state
            ),
            patch.object(
                rename_mod._shared.cache, "get_state_ref", return_value=fake_state
            ),
            ...
            result = asyncio.run(
                rename_mod.rename_track(
                    "test-album", "01-old-track", "01-renamed-track"
                )
            )
        ...
        with (
            ...
            patch.object(
                ...
                side_effect=lambda name: (
                    fake_prepare_mod if name == "prepare_singles" else fake_songbook_mod
                ),
            ),
