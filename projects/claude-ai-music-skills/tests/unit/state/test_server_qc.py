...
class TestQcAudioComprehensive:
    ...
    def _mock_qc_result(
        self, filename, verdict="PASS", phase_status="PASS", spectral_status="PASS"
    ):
        return {
            "filename": filename,
            "checks": {
    ...
    def test_subfolder_resolves(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.qc_audio("test-album", subfolder="mastered"))
            )
...
class TestMasterAlbumPipeline:
    ...
    def test_pre_qc_skips_truepeak_and_clicks(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.mastering.master_tracks.load_genre_presets", return_value={}
        ), patch(
    ...
    def test_settings_returned_in_response(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.mastering.master_tracks.load_genre_presets",
            return_value={
                "rock": {
                    "target_lufs": -14.0,
                    "cut_highmid": -2.5,
    ...
    def test_auto_recovery_triggers_on_dynamic_range(self, tmp_path):
        ...
        def mock_fix_dynamic(
            data, rate, target_lufs=-14.0, eq_settings=None, ceiling_db=-1.0
        ):
        ...
        assert (
            result["stages"]["verification"]["auto_recovered"][0]["filename"]
            == "01-track-1.wav"
        )
    ...
    def test_auto_recovery_reported_in_warnings(self, tmp_path):
        ...
        recovery_warnings = [
            ...
            if isinstance(w, dict) and w.get("type") == "auto_recovery"
        ]
...
class TestResetMastering:
    ...
    def test_dry_run_reports_without_deleting(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.reset_mastering("test-album", dry_run=True))
            )
    ...
    def test_delete_mastered(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.reset_mastering("test-album", dry_run=False))
            )
    ...
    def test_rejects_disallowed_subfolders(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.reset_mastering("test-album", subfolders=["originals"]))
            )
        assert "error" in result
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.reset_mastering("test-album", subfolders=["stems"]))
            )
        assert "error" in result

        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.reset_mastering("test-album", subfolders=["foo"]))
            )
        assert "error" in result
...
class TestMasterAlbumStaging:
    ...
    def test_successful_mastering_populates_mastered_dir(self, tmp_path):
        ...
        mastered_wavs = sorted(
            f.name for f in mastered_dir.iterdir() if f.suffix == ".wav"
        )
...
def _write_inverted_phase_wav(path) -> None:
    ...
    fmt = struct.pack(
        "<4sIHHIIHH", b"fmt ", 16, 1, n_channels, rate, byte_rate, block_align, bits
    )
    data = struct.pack("<4sI", b"data", len(interleaved)) + interleaved
    riff = (
        struct.pack("<4sI4s", b"RIFF", 4 + len(fmt) + len(data), b"WAVE") + fmt + data
    )
    with open(str(path), "wb") as f:
        f.write(riff)
