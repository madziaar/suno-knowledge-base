...
class TestReExportCompleteness:
    ...
    def test_all_registered_tools_are_reexported(self):
        ...
        missing = [name for name in registered_names if not hasattr(server, name)]
        assert not missing, (
...
class TestStateCacheGetState:
    ...
    def test_returns_cached_state_when_not_stale(self):
        ...
        with patch.object(server, "STATE_FILE", mock_state), patch.object(
            server, "CONFIG_FILE", mock_config
...
class TestStateCacheRebuild:
    ...
    def test_rebuild_success(self):
        ...
        with patch.object(server, "STATE_FILE", mock_state), patch.object(
            server, "CONFIG_FILE", mock_config
        ), patch.object(server, "read_config", return_value=config), patch.object(
            server, "read_state", return_value=existing_state
        ), patch.object(
            server, "build_state", return_value=new_state
        ) as mock_build, patch.object(server, "write_state") as mock_write:
    ...
    def test_rebuild_build_failure(self):
        cache = server.StateCache()
        with patch.object(
            server, "read_config", return_value={"artist": {"name": "test"}}
        ), patch.object(server, "read_state", return_value=None), patch.object(
            server, "build_state", side_effect=RuntimeError("glob failed")
        ):
            result = cache.rebuild()
        assert "error" in result
...
class TestStateCacheUpdateSession:
    ...
    def test_set_album(self):
        ...
        with patch.object(server, "STATE_FILE", ms), patch.object(
            server, "CONFIG_FILE", mc
...
class TestGetPendingVerifications:
    ...
    def test_none_pending(self):
        ...
        state["albums"]["test-album"]["tracks"]["02-second-track"][
            "sources_verified"
        ] = "Verified"
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache):
    ...
    def test_multiple_albums_with_pending(self):
        ...
        state = _fresh_state()
        state["albums"]["another-album"]["tracks"]["01-rock-song"][
            "sources_verified"
        ] = "Pending"
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache):
    ...
    def test_album_slug_filters_to_one_album(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_pending_verifications(album_slug="test-album"))
            )
        assert result["total_pending_tracks"] == 1
        assert "test-album" in result["albums_with_pending"]
    ...
    def test_album_slug_normalizes_input(self):
        ...
        for variant in ("Test-Album", "test_album", "TEST ALBUM"):
            with patch.object(_shared_mod, "cache", mock_cache):
                result = json.loads(
                    _run(server.get_pending_verifications(album_slug=variant))
                )
            assert result["total_pending_tracks"] == 1, (
    ...
    def test_album_slug_no_match(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_pending_verifications(album_slug="nonexistent"))
            )
        assert result["total_pending_tracks"] == 0
    ...
    def test_summary_only_returns_counts(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_pending_verifications(summary_only=True))
            )
        assert result["total_pending_tracks"] == 1
    ...
    def test_summary_only_false_default(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_pending_verifications(summary_only=False))
            )
    ...
    def test_summary_only_with_album_slug(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.get_pending_verifications(
...
class TestResolvePath:
    ...
    def test_audio_path_with_explicit_genre(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.resolve_path("audio", "test-album", genre="rock"))
            )
        assert (
            result["path"]
    ...
    def test_content_path_with_genre(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.resolve_path("content", "test-album", genre="electronic"))
            )
        assert (
            result["path"]
...
class TestResolveTrackFile:
    ...
    def test_exact_match(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.resolve_track_file("test-album", "01-first-track"))
            )
        assert result["found"] is True
    ...
    def test_track_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.resolve_track_file("test-album", "99-missing"))
            )
        assert result["found"] is False
        assert "available_tracks" in result
...
class TestListTrackFiles:
    ...
    def test_status_filter(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.list_track_files("test-album", status_filter="Final"))
            )
    ...
    def test_status_filter_no_match(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.list_track_files("test-album", status_filter="Generated"))
            )
...
class TestExtractSection:
    ...
    def test_extract_style_box(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "style"))
            )
        assert result["found"] is True
    ...
    def test_extract_lyrics_box(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "lyrics"))
            )
        assert result["found"] is True
    ...
    def test_extract_streaming_lyrics(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "streaming"))
            )
        assert result["found"] is True
    ...
    def test_extract_exclude_styles(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "exclude-styles"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_extract_exclude_styles_short_alias(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "exclude"))
            )
        assert result["found"] is True
        assert result["section"] == "Exclude Styles"
        assert _SAMPLE_EXCLUDE_CONTENT == result["content"]
    ...
    def test_extract_concept(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "concept"))
            )
        assert result["found"] is True
    ...
    def test_extract_pronunciation(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "pronunciation"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_extract_musical_direction(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "musical-direction"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_extract_production_notes(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "production-notes"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_unknown_section(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section("test-album", "01-test-track", "nonexistent")
                )
            )
        assert "error" in result
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("nonexistent", "01", "lyrics"))
            )
        assert result["found"] is False
    ...
    def test_track_prefix_match(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "05", "concept"))
            )
        assert result["found"] is True
    ...
    def test_missing_section_in_file(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "source"))
            )
        assert result["found"] is False
...
class TestUpdateTrackField:
    ...
    def test_update_status(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            server, "write_state", MagicMock()
        ):
            result = json.loads(
                _run(
                    server.update_track_field(
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_track_field("nonexistent", "01", "status", "Final"))
            )
        assert result["found"] is False
    ...
    def test_state_cache_updated(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _core_mod, "write_state", mock_write
        ):
            _run(
                server.update_track_field(
                    "test-album", "01-test-track", "status", "Generated", force=True
        ...
        assert (
            state["albums"]["test-album"]["tracks"]["01-test-track"]["status"]
            == "Generated"
        )
    ...
    def test_update_read_only_file(self, tmp_path):
        ...
        mock_cache, track_file = self._make_cache_with_file(tmp_path)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _core_mod, "atomic_write_text", side_effect=OSError("Permission denied")
        ):
            result = json.loads(
                _run(
                    server.update_track_field(
    ...
    def test_rapid_sequential_updates(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            _run(
                server.update_track_field(
                    "test-album", "01-test-track", "suno-link", "https://suno.com/test"
                )
            )
...
class TestGetReference:
    ...
    def test_with_section(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", tmp_path):
            result = json.loads(
                _run(server.get_reference("guide", section="Section A"))
            )
        assert result["found"] is True
...
class TestFormatForClipboard:
    ...
    def test_lyrics(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.format_for_clipboard("test-album", "01-test-track", "lyrics")
                )
            )
        assert result["found"] is True
        assert "[Verse 1]" in result["content"]
    ...
    def test_style(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.format_for_clipboard("test-album", "01-test-track", "style")
                )
            )
        assert result["found"] is True
        assert "electronic" in result["content"]
    ...
    def test_streaming(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.format_for_clipboard(
                        "test-album", "01-test-track", "streaming"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_all_combined(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "01-test-track", "all"))
            )
        assert result["found"] is True
    ...
    def test_invalid_type(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "01", "invalid"))
            )
        assert "error" in result
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("nonexistent", "01", "lyrics"))
            )
        assert result["found"] is False
    ...
    def test_ambiguous_prefix_returns_error(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "01", "style"))
            )
    ...
    def test_unique_prefix(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "05", "lyrics"))
            )
        assert result["found"] is True
    ...
    def test_track_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "99-missing", "lyrics"))
            )
        assert result["found"] is False
        assert "not found" in result["error"]
    ...
    def test_missing_section_in_track(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "05", "streaming"))
            )
        assert result["found"] is False
        assert "not found" in result["error"]
    ...
    def test_track_no_path(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "05-no-path", "lyrics"))
            )
        assert "error" in result
...
class TestScanArtistNames:
    ...
    def test_finds_blocked_name(self):
        ...
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(
                    server.scan_artist_names(
                        "aggressive dubstep like Skrillex with heavy drops"
                    )
                )
            )
    ...
    def test_clean_text(self):
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(
                    server.scan_artist_names(
                        "aggressive dubstep, heavy drops, distorted bass"
                    )
                )
            )
    ...
    def test_multiple_names(self):
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(server.scan_artist_names("mix of Eminem and Drake style"))
            )
        names = [r["name"] for r in result["matches"]]
    ...
    def test_word_boundary_no_partial(self):
        ...
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(server.scan_artist_names("Drakeford is not a musician"))
            )
    ...
    def test_blocklist_file_missing(self, tmp_path):
        ...
        with patch.object(
            _text_analysis_mod, "_artist_blocklist_cache", None
    ...
    def test_special_characters_near_name(self):
        ...
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(server.scan_artist_names("like (Skrillex) or [Deadmau5]"))
            )
        names = [r["name"] for r in result["matches"]]
        assert "Skrillex" in names
...
class TestCheckPronunciationEnforcement:
    ...
    def test_all_applied(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.check_pronunciation_enforcement(
                        "test-album", "05-pron-track"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_unapplied_entries(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.check_pronunciation_enforcement("test-album", "05-unapplied")
                )
            )
    ...
    def test_empty_pronunciation_table(self, tmp_path):
        ...
        track_file.write_text(
            _SAMPLE_TRACK_MD
        )  # has pronunciation table with only "—" placeholder
        state = _fresh_state()
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("nonexistent", "01"))
            )
        assert result["found"] is False
    ...
    def test_prefix_match(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("test-album", "05"))
            )
        assert result["found"] is True
    ...
    def test_track_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("test-album", "99-missing"))
            )
        assert result["found"] is False
        assert "not found" in result["error"]
    ...
    def test_no_pronunciation_section(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("test-album", "05-no-pron"))
            )
        assert result["found"] is True
...
class TestGetAlbumFull:
    ...
    def test_with_sections(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.get_album_full("test-album", include_sections="lyrics,style")
                )
            )
    ...
    def test_invalid_section_ignored(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.get_album_full(
                        "test-album", include_sections="lyrics,invalid-section"
                    )
                )
            )
        test_track = result["tracks"].get("01-test-track", {})
    ...
    def test_track_file_missing_on_disk(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_album_full("test-album", include_sections="lyrics"))
            )
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.get_album_full(
                        "test-album",
                        track_slugs="01-first-track,02-second-track",
                    )
                )
            )
        assert "01-first-track" in result["tracks"]
    ...
    def test_track_slugs_empty_returns_all(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_album_full("test-album", track_slugs=""))
            )
...
class TestValidateAlbumStructure:
    ...
    def _make_album_on_disk(self, tmp_path):
        ...
        album_dir = (
            content / "artists" / "test-artist" / "albums" / "electronic" / "test-album"
        )
        tracks_dir = album_dir / "tracks"
        audio_dir = (
            audio / "artists" / "test-artist" / "albums" / "electronic" / "test-album"
        )
    ...
    def test_missing_tracks_dir(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.validate_album_structure("test-album", checks="structure"))
            )
    ...
    def test_audio_wrong_location(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.validate_album_structure("test-album", checks="audio"))
            )
        failed_msgs = [c["message"] for c in result["checks"] if c["status"] == "FAIL"]
    ...
    def test_specific_checks(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.validate_album_structure("test-album", checks="art"))
            )
    ...
    def test_track_validation(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.validate_album_structure("test-album", checks="tracks"))
            )
    ...
    def test_permission_denied_tracks_dir(self, tmp_path):
        ...
        try:
            with patch.object(_shared_mod, "cache", mock_cache):
                result = json.loads(
                    _run(
                        server.validate_album_structure(
                            "test-album", checks="structure"
                        )
                    )
                )
            assert result["found"] is True
    ...
    def test_multiple_check_types(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.validate_album_structure(
                        "test-album", checks="structure,art"
                    )
                )
            )
        categories = set(c["category"] for c in result["checks"])
    ...
    def test_invalid_check_type_ignored(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.validate_album_structure("test-album", checks="invalid"))
            )
        categories = set(c["category"] for c in result["checks"])
...
class TestCreateAlbumStructure:
    ...
    def test_create_basic(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_album_structure("new-album", "electronic"))
            )
    ...
    def test_create_documentary(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(
                    server.create_album_structure(
                        "doc-album", "hip-hop", documentary=True
                    )
                )
            )
        assert result["created"] is True
    ...
    def test_slug_normalization(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_album_structure("My New Album", "Hip Hop"))
            )
        assert result["created"] is True
    ...
    def test_missing_templates_graceful(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _shared_mod, "PLUGIN_ROOT", fake_plugin
        ):
            result = json.loads(
                _run(server.create_album_structure("no-templates", "rock"))
            )
        assert result["created"] is True
...
class TestGenreValidation:
    ...
    def test_valid_genre_accepted(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_album_structure("genre-test", "electronic"))
            )
        assert result["created"] is True
    ...
    def test_invalid_genre_rejected(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_album_structure("genre-test", "not-a-real-genre"))
            )
    ...
    def test_genre_typo_rejected(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_album_structure("typo-test", "elctronic"))
            )
        _shared_mod._VALID_GENRES = None
        assert "error" in result
        assert "Invalid genre" in result["error"]
    ...
    def test_additional_genre_from_config(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_album_structure("synth-test", "synthwave"))
            )
        assert result["created"] is True
...
class TestRunPreGenerationGates:
    ...
    def test_all_pass(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05"))
            )
        assert result["found"] is True
        track = result["tracks"][0]
    ...
    def test_multiple_failures(self, tmp_path):
        ...
        assert (
            track["blocking"] >= 3
        )  # sources, TODO markers, pronunciation, artist names
    ...
    def test_track_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "99"))
            )
        assert result["found"] is False
    ...
    def test_track_no_file_path(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05-no-file"))
            )
        track = result["tracks"][0]
        # Pronunciation should SKIP because no file text
        pron_gate = next(
            g for g in track["gates"] if g["gate"] == "Pronunciation Resolved"
        )
    ...
    def test_explicit_flag_true_passes(self, tmp_path):
        ...
        explicit_gate = next(
            g for g in result["tracks"][0]["gates"] if g["gate"] == "Explicit Flag Set"
        )
    ...
    def test_unreadable_track_file(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05-unreadable"))
            )
        assert result["found"] is True
        track = result["tracks"][0]
    ...
    def test_permission_error_track_file(self, tmp_path):
        ...
        try:
            with patch.object(_shared_mod, "cache", mock_cache), patch.object(
                ...
                result = json.loads(
                    _run(server.run_pre_generation_gates("test-album", "05-denied"))
                )
            assert result["found"] is True
            track = result["tracks"][0]
            assert len(track["gates"]) == 8
    ...
    def test_sources_verified_case_variations(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05-src"))
            )
        src_gate = next(
            g for g in result["tracks"][0]["gates"] if g["gate"] == "Sources Verified"
        )
...
class TestHomographGate:
    ...
    def test_gate7_pass_no_homographs(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05-clean"))
            )
        homograph_gate = next(
            g for g in result["tracks"][0]["gates"] if g["gate"] == "Homograph Check"
        )
    ...
    def test_gate7_fail_homograph_in_lyrics(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05-homo"))
            )
        homograph_gate = next(
            g for g in result["tracks"][0]["gates"] if g["gate"] == "Homograph Check"
        )
    ...
    def test_gate7_skip_no_lyrics(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "05-no-lyrics"))
            )
        homograph_gate = next(
            g for g in result["tracks"][0]["gates"] if g["gate"] == "Homograph Check"
        )
...
class TestPreGenGateEnforcementInUpdateTrackField:
    ...
    def test_generated_blocked_homograph(self, tmp_path):
        ...
        content = _TRACK_ALL_GATES_PASS.replace(
            "Testing one two three", "We live for the night"
        )
        mock_cache, _ = self._make_cache_with_track(tmp_path, content)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _text_analysis_mod, "_artist_blocklist_cache", None
    ...
    def test_lyric_length_passes_under_800(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.run_pre_generation_gates("test-album", "01-test-track"))
            )
        length_gate = next(
            g for g in result["tracks"][0]["gates"] if g["gate"] == "Lyric Length"
        )
    ...
    def test_lyric_length_blocks_over_800(self, tmp_path):
        ...
        long_lyrics = "\n".join(
            f"word{i} word{i}a word{i}b word{i}c word{i}d" for i in range(170)
        )
        content = _TRACK_ALL_GATES_PASS.replace(
            "[Verse 1]\nTesting one two three\nThis is a test for me\n\n"
            "[Chorus]\nWe're testing all day long\nTesting in this song",
...
class TestCheckExplicitContent:
    ...
    def test_override_adds_words(self, tmp_path):
        ...
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _text_analysis_mod, "_explicit_word_cache", None
        ):
            result = json.loads(
                _run(server.check_explicit_content("This has a customword"))
            )
        assert result["has_explicit"] is True
    ...
    def test_override_error_logs_warning(self, tmp_path, caplog):
        ...
        assert any(
            "explicit word overrides" in r.message.lower()
            or "Failed to load" in r.message
    ...
    def test_section_tag_with_dash_not_scanned(self):
        ...
        with patch.object(_text_analysis_mod, "_explicit_word_cache", None):
            result = json.loads(
                _run(server.check_explicit_content("[Fuck-Chorus]\nClean lyrics"))
            )
        assert result["has_explicit"] is False
    ...
    def test_explicit_word_at_line_start(self):
        ...
        with patch.object(_text_analysis_mod, "_explicit_word_cache", None):
            result = json.loads(
                _run(server.check_explicit_content("Shit, that was loud"))
            )
        assert result["has_explicit"] is True
    ...
    def test_explicit_word_at_line_end(self):
        ...
        with patch.object(_text_analysis_mod, "_explicit_word_cache", None):
            result = json.loads(
                _run(server.check_explicit_content("Oh that's some shit"))
            )
        assert result["has_explicit"] is True
    ...
    def test_override_adds_and_removes_combined(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result2 = json.loads(
                _run(server.check_explicit_content("That's a badterm"))
            )
...
class TestExtractLinks:
    ...
    def test_file_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_links("test-album", "NONEXISTENT.md"))
            )
        assert result["found"] is False
...
class TestGetLyricsStats:
    ...
    def test_track_no_path(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_lyrics_stats("test-album", "05-no-path"))
            )
...
class TestListSkills:
    ...
    def test_combined_model_and_category(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.list_skills(model_filter="sonnet", category="court"))
            )
        assert result["count"] == 1
        assert result["skills"][0]["name"] == "researchers-legal"
    ...
    def test_combined_filter_no_match(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.list_skills(model_filter="opus", category="court"))
            )
        assert result["count"] == 0
...
class TestUpdateAlbumStatus:
    ...
    def _make_cache_with_readme(self, tmp_path):
        ...
        state["albums"]["test-album"]["tracks"]["02-second-track"]["status"] = (
            "Generated"
        )
    ...
    def test_invalid_status_rejected(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("test-album", "InvalidStatus"))
            )
        assert "error" in result
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("no-such-album", "Complete"))
            )
        assert result["found"] is False
    ...
    def test_all_valid_statuses(self, tmp_path):
        ...
        for status in [
            ...
            with patch.object(_shared_mod, "cache", mock_cache), patch.object(
                ...
                result = json.loads(
                    _run(server.update_album_status("test-album", status, force=True))
                )
    ...
    def test_write_oserror(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _status_mod, "atomic_write_text", side_effect=OSError("disk full")
        ):
            result = json.loads(
                _run(server.update_album_status("test-album", "Complete"))
            )
    ...
    def test_cache_update_failure_still_succeeds(self, tmp_path):
        ...
        mock_cache, readme_path = self._make_cache_with_readme(tmp_path)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _parsers_mod, "parse_album_readme", side_effect=Exception("parse fail")
        ), patch.object(server, "write_state"):
            result = json.loads(
                _run(server.update_album_status("test-album", "Complete"))
            )
...
class TestCreateTrack:
    ...
    def test_creates_track_file(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_track("test-album", "03", "New Song"))
            )
        assert result["created"] is True
    ...
    def test_title_slugified(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_track("test-album", "01", "My Cool Track"))
            )
    ...
    def test_missing_template(self, tmp_path):
        ...
        empty_root.mkdir()
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _shared_mod, "PLUGIN_ROOT", empty_root
        ):
            result = json.loads(_run(server.create_track("test-album", "01", "Track")))
        assert "error" in result
    ...
    def test_special_chars_in_title(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_track("test-album", "01", "The Big Track"))
            )
        assert result["created"] is True
    ...
    def test_documentary_true_keeps_source_sections(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(
                    server.create_track(
                        "test-album", "01", "Doc Track", documentary=True
                    )
                )
            )
        assert result["created"] is True
    ...
    def test_documentary_false_strips_source_sections(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(
                    server.create_track(
                        "test-album", "01", "Non Doc", documentary=False
                    )
                )
            )
        assert result["created"] is True
    ...
    def test_template_read_oserror(self, tmp_path):
        ...
        try:
            with patch.object(_shared_mod, "cache", mock_cache), patch.object(
                ...
                result = json.loads(
                    _run(server.create_track("test-album", "01", "Track"))
                )
            assert "error" in result
            assert "Cannot read" in result["error"]
        finally:
    ...
    def test_fills_frontmatter_track_number(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_track("test-album", "07", "My Track"))
            )
        assert result["created"] is True
        content = (tracks_dir / "07-my-track.md").read_text()
    ...
    def test_fills_frontmatter_explicit(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_track("test-album", "01", "Explicit Track"))
            )
        assert result["created"] is True
...
class TestGetPromoStatus:
    ...
    def test_populated_file_detected(self, tmp_path):
        ...
        mock_cache, promo_dir = self._make_cache_with_promo(
            tmp_path,
            {
...
class TestGetPromoContent:
    ...
    def test_invalid_platform(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_promo_content("test-album", "snapchat"))
            )
        assert "error" in result
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_promo_content("no-such-album", "twitter"))
            )
        assert result["found"] is False
    ...
    def test_file_read_oserror(self, tmp_path):
        ...
        try:
            with patch.object(_shared_mod, "cache", mock_cache):
                result = json.loads(
                    _run(server.get_promo_content("test-album", "twitter"))
                )
            assert "error" in result
            assert "Cannot read" in result["error"]
        finally:
    ...
    def test_returns_path_and_platform(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.get_promo_content("test-album", "instagram"))
            )
...
class TestCheckVenvHealth:
    ...
    def test_all_match_returns_ok(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", tmp_path), patch(
    ...
    def test_version_mismatch_returns_stale(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", tmp_path), patch(
            "importlib.metadata.version", return_value="2.28.0"
        ), patch.object(Path, "home", return_value=tmp_path / "fakehome"):
            result = json.loads(_run(server.check_venv_health()))
    ...
    def test_missing_requirements_returns_error(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", tmp_path), patch.object(
            Path, "home", return_value=tmp_path / "fakehome"
        ):
            result = json.loads(_run(server.check_venv_health()))
...
class TestCheckSkillRegistration:
    ...
    def _setup_skills(
        self, tmp_path, source_skills, cached_skills, cached_version="0.89.0"
    ):
    ...
    def test_all_match_returns_ok(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", plugin_root), patch.object(
            Path, "home", return_value=tmp_path / "fakehome"
        ):
    ...
    def test_no_cache_returns_no_cache(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", plugin_root), patch.object(
            Path, "home", return_value=fakehome
        ):
            result = _health_mod._check_skill_registration()
    ...
    def test_cached_version_reported(self, tmp_path):
        ...
        skills = ["test-skill"]
        plugin_root = self._setup_skills(
            tmp_path, skills, skills, cached_version="0.69.0"
        )
...
class TestHealthCheck:
    ...
    def test_all_ok(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", plugin_root), patch(
            "importlib.metadata.version", side_effect=mock_version
        ), patch.object(Path, "home", return_value=tmp_path / "fakehome"):
    ...
    def test_stale_skills_returns_warn(self, tmp_path):
        ...
        with patch.object(_shared_mod, "PLUGIN_ROOT", plugin_root), patch(
            "importlib.metadata.version", return_value="2.31.0"
        ), patch.object(Path, "home", return_value=tmp_path / "fakehome"):
            result = json.loads(_run(_health_mod.health_check()))
...
class TestCreateIdea:
    ...
    def test_cache_rebuild_failure_still_succeeds(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.create_idea("Survive Rebuild", genre="rock"))
            )
        assert result["created"] is True
        text = (content_root / "IDEAS.md").read_text()
...
"""
    ...
    def test_updates_status(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Cyberpunk Dreams", "status", "In Progress"))
            )
        assert result["success"] is True
    ...
    def test_updates_genre(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Cyberpunk Dreams", "genre", "synthwave"))
            )
        assert result["success"] is True
        text = (content_root / "IDEAS.md").read_text()
    ...
    def test_idea_not_found(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("No Such Idea", "status", "Pending"))
            )
        assert result["found"] is False
    ...
    def test_invalid_field(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Cyberpunk Dreams", "tracks", "12"))
            )
        assert "error" in result
        assert "Unknown field" in result["error"]
    ...
    def test_updates_concept(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.update_idea(
                        "Cyberpunk Dreams", "concept", "Dark future vibes"
                    )
                )
            )
        assert result["success"] is True
    ...
    def test_updates_type_field(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Cyberpunk Dreams", "type", "Documentary"))
            )
        assert result["success"] is True
        text = (content_root / "IDEAS.md").read_text()
        assert "**Type**: Documentary" in text
...
"""
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Minimal Idea", "genre", "rock"))
            )
        assert "error" in result
    ...
    def test_last_idea_in_file(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Outlaw Stories", "status", "Complete"))
            )
        assert result["success"] is True
        text = (content_root / "IDEAS.md").read_text()
    ...
    def test_empty_value(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_idea("Cyberpunk Dreams", "concept", ""))
            )
        assert result["success"] is True
        text = (content_root / "IDEAS.md").read_text()
    ...
    def test_ideas_read_oserror(self, tmp_path):
        ...
        try:
            with patch.object(_shared_mod, "cache", mock_cache):
                result = json.loads(
                    _run(server.update_idea("Test", "status", "Pending"))
                )
            assert "error" in result
            assert "Cannot read" in result["error"]
        finally:
...
class TestLoadExplicitWords:
    ...
    def test_cache_hit_returns_immediately(self):
        ...
        mock_cache = MockStateCache()
        with patch.object(
            _text_analysis_mod, "_explicit_word_cache", cached_words
        ), patch.object(_text_analysis_mod, "_explicit_word_mtime", 0.0), patch.object(
...
class TestLoadArtistBlocklist:
    ...
    def test_cache_hit_returns_immediately(self):
        ...
        blocklist_path = (
            server.PLUGIN_ROOT / "reference" / "suno" / "artist-blocklist.md"
        )
        try:
            ...
            mtime = 0.0
        with patch.object(
            _text_analysis_mod, "_artist_blocklist_cache", cached
    ...
    def test_missing_file_returns_empty(self, tmp_path):
        ...
        with patch.object(
            ...
            _text_analysis_mod, "_artist_blocklist_patterns", None
        ), patch.object(_shared_mod, "PLUGIN_ROOT", tmp_path):
            result = _text_analysis_mod._load_artist_blocklist()
        assert result == []
...
class TestCreateIdeaFormatting:
    ...
    def test_idea_block_format(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _ideas_mod, "_resolve_ideas_path", return_value=ideas_file
        ):
            result = json.loads(
                _run(
                    server.create_idea(
...
class TestCheckPronunciationEdgeCases:
    ...
    def test_word_in_data_row_skipped(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("test-album", "01-test"))
            )
    ...
    def test_no_pronunciation_section(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("test-album", "01-simple"))
            )
    ...
    def test_unapplied_pronunciation(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_pronunciation_enforcement("test-album", "01-pron"))
            )
...
class TestGetAlbumFullEdgeCases:
    ...
    def test_with_section_extraction(self, tmp_path):
        ...
        state = _fresh_state()
        state["albums"]["test-album"]["tracks"]["01-first-track"]["path"] = str(
            track_file
        )
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache):
...
class TestUpdateAlbumStatusEdgeCases:
    ...
    def test_invalid_status_rejected(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("test-album", "Invalid Status"))
            )
        assert "error" in result
        assert "Invalid status" in result["error"]
...
class TestAlbumTrackConsistencyCheck:
    ...
    def test_in_progress_blocked_all_not_started(self, tmp_path):
        ...
        mock_cache, _ = self._make_cache_with_tracks(
            tmp_path,
            "Concept",
            {
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("test-album", "In Progress"))
            )
        assert "error" in result
    ...
    def test_force_bypasses_consistency_check(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.update_album_status("test-album", "Complete", force=True))
            )
        assert result["success"] is True
    ...
    def test_early_statuses_no_track_requirements(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.update_album_status("test-album", "Research Complete"))
            )
        assert result["success"] is True
...
class TestReleaseReadinessGate:
    ...
    def _make_release_env(
        self, tmp_path, has_audio=True, has_mastered=True, has_art=True
    ):
    ...
    def test_released_blocked_no_audio(self, tmp_path):
        ...
        mock_cache = self._make_release_env(
            tmp_path, has_audio=False, has_mastered=False, has_art=False
        )
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("test-album", "Released"))
    ...
    def test_force_bypasses_release_gate(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.update_album_status("test-album", "Released", force=True))
            )
        assert result["success"] is True
...
class TestDocumentaryAlbumSourcePath:
    ...
    def test_concept_to_in_progress_blocked_with_sources(self, tmp_path):
        ...
        mock_cache = self._make_cache_with_album(
            tmp_path, has_sources=True, status="Concept"
        )
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("test-album", "In Progress"))
    ...
    def test_concept_to_in_progress_allowed_without_sources(self, tmp_path):
        ...
        mock_cache = self._make_cache_with_album(
            tmp_path, has_sources=False, status="Concept"
        )
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            server, "write_state"
        ):
    ...
    def test_force_bypasses_documentary_check(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(
                    server.update_album_status("test-album", "In Progress", force=True)
                )
            )
        assert result["success"] is True
...
class TestValidateAlbumStructureEdgeCases:
    ...
    def test_empty_check_set_defaults_to_all(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.validate_album_structure("test-album", checks=""))
            )
        assert result["found"] is True
...
class TestExtractSectionNewSectionNames:
    ...
    def test_extract_mood_section(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.extract_section("test-album", "01-test-track", "mood"))
            )
        assert result["found"] is True
    ...
    def test_extract_mood_imagery_alias(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "mood-imagery"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_extract_lyrical_approach(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "lyrical-approach"
                    )
                )
            )
        assert result["found"] is True
    ...
    def test_extract_phonetic_review(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.extract_section(
                        "test-album", "01-test-track", "phonetic-review"
                    )
                )
            )
        assert result["found"] is True
...
class TestAlbumStatusTransitionEnforcement:
    ...
    def _make_cache_with_album_status(
        self, tmp_path, album_status, track_statuses=None
    ):
    ...
    def test_sources_verified_gate_blocks_unverified_tracks(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.update_album_status("test-album", "Sources Verified"))
            )
        assert "error" in result
...
class TestFormatForClipboardEdgeCasesRound5:
    ...
    def test_streaming_lyrics_alias(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.format_for_clipboard("test-album", "05", "streaming-lyrics")
                )
            )
        assert result["found"] is True
...
class TestFormatForClipboardSuno:
    ...
    def test_suno_returns_json_with_title_style_lyrics(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "01-test-track", "suno"))
            )
        assert result["found"] is True
    ...
    def test_suno_exclude_styles_empty_when_missing(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "05", "suno"))
            )
        assert result["found"] is True
        payload = json.loads(result["content"])
    ...
    def test_all_content_type_omits_exclude_when_empty(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "05", "all"))
            )
        assert result["found"] is True
    ...
    def test_style_without_exclude_returns_style_only(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.format_for_clipboard("test-album", "05", "style"))
            )
        assert result["found"] is True
    ...
    def test_exclude_content_type(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.format_for_clipboard(
                        "test-album", "01-test-track", "exclude"
                    )
                )
            )
        assert result["found"] is True
...
class TestSourceVerificationGate:
    ...
    def _make_cache_with_track_and_album(
        self, tmp_path, track_content, album_has_sources=False
    ):
    ...
    def test_pending_value_not_blocked(self, tmp_path):
        ...
        for val in ["❌ Pending", "N/A", "Pending"]:
            ...
            assert result.get(
                "success"
            ) is True or "no markdown links" not in result.get("error", "")
...
class TestSectionNamesCompleteness:
    ...
    def test_bidirectional_aliases_consistent(self):
        ...
        assert (
            server._SECTION_NAMES["streaming"]
            == server._SECTION_NAMES["streaming-lyrics"]
        )
        assert (
            server._SECTION_NAMES["pronunciation"]
            == server._SECTION_NAMES["pronunciation-notes"]
        )
        assert server._SECTION_NAMES["mood"] == server._SECTION_NAMES["mood-imagery"]
        assert (
            server._SECTION_NAMES["exclude"] == server._SECTION_NAMES["exclude-styles"]
        )
...
class TestDeriveTitleFromSlug:
    ...
    def test_multi_hyphen_slug(self):
        ...
        assert (
            server._derive_title_from_slug("03-the-long-and-winding-road")
...
class TestRenameAlbum:
    ...
    def test_rename_album_auto_title(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.rename_album("test-album", "cool-new-name"))
            )
    ...
    def test_rename_album_already_exists(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.rename_album("test-album", "another-album"))
            )
        assert "error" in result
        assert "already exists" in result["error"]
    ...
    def test_rename_album_normalizes_slugs(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.rename_album("test album", "new_album_name"))
            )
        assert result["success"] is True
        assert result["old_slug"] == "test-album"
    ...
    def test_rename_album_state_cache_updated(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _indexer, "write_state", mock_write
        ):
            _run(server.rename_album("test-album", "new-album"))
        albums = mock_cache.get_state()["albums"]
        assert "test-album" not in albums
...
class TestRenameTrack:
    ...
    def test_rename_track_success(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.rename_track("test-album", "01-old-track", "01-new-track"))
            )
        assert result["success"] is True
    ...
    def test_rename_track_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.rename_track("test-album", "99-missing", "99-new-name"))
            )
        assert "error" in result
        assert "not found" in result["error"].lower()
    ...
    def test_rename_track_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.rename_track("nonexistent-album", "01-track", "01-new-track")
                )
            )
        assert "error" in result
        assert "not found" in result["error"]
    ...
    def test_rename_track_already_exists(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.rename_track("test-album", "01-old-track", "02-existing"))
            )
        assert "error" in result
        assert "already exists" in result["error"]
    ...
    def test_rename_track_prefix_match(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.rename_track("test-album", "01", "01-new-track"))
            )
        assert result["success"] is True
        assert result["old_slug"] == "01-old-track"
    ...
    def test_rename_track_normalizes_slugs(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.rename_track("test-album", "01_old_track", "01_new_track"))
            )
        assert result["success"] is True
        assert result["old_slug"] == "01-old-track"
        assert result["new_slug"] == "01-new-track"
...
class TestPathTraversalGuards:
    ...
    def test_subfolder_traversal_rejected(self, tmp_path):
        ...
        result = json.loads(err)
        assert (
            "escape" in result["error"].lower() or "invalid" in result["error"].lower()
        )
    ...
    def test_master_audio_source_subfolder_traversal(self, tmp_path):
        ...
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_mastering_deps", return_value=None
        ):
            result = json.loads(
                _run(
                    server.master_audio(
                        "test-album",
                        source_subfolder="../../etc",
                    )
                )
            )
        assert "error" in result
    ...
    def test_master_with_reference_traversal(self, tmp_path):
        ...
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_matchering", return_value=None
        ):
            result = json.loads(
                _run(
                    server.master_with_reference(
                        "test-album",
                        reference_filename="../../etc/passwd",
                    )
                )
            )
        assert "error" in result
    ...
    def test_master_with_reference_target_traversal(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "handlers.processing.audio._ref_master", mock_ref_master, create=True
    ...
    def test_promo_videos_track_traversal(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_ffmpeg", return_value=None
        ), patch.dict(
            "sys.modules",
            {
                "tools.promotion.generate_promo_video": mock_promo_mod,
                "tools.shared.fonts": MagicMock(
                    find_font=MagicMock(return_value="/fake/font.ttf")
                ),
            },
        ):
            result = json.loads(
                _run(
                    server.generate_promo_videos(
                        "test-album",
                        track_filename="../../etc/passwd",
                    )
                )
            )
        assert "error" in result
    ...
    def test_transcribe_track_traversal(self, tmp_path):
        ...
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_anthemscore", return_value=None
        ), patch.object(
            _processing_helpers,
...
class TestAnalyzeAudio:
    ...
    def test_missing_deps_returns_error(self):
        with patch.object(
            _processing_helpers, "_check_mastering_deps", return_value="Missing deps"
        ):
    ...
    def test_successful_analysis(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.mastering.analyze_tracks.analyze_track", return_value=mock_result
        ):
            result = json.loads(_run(server.analyze_audio("test-album")))
    ...
    def test_subfolder_param(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.analyze_audio("test-album", subfolder="mastered"))
            )
        assert "tracks" in result
...
class TestMasterAudio:
    ...
    def test_unknown_genre(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.master_audio("test-album", genre="nonexistent-genre"))
            )
        assert "error" in result
    ...
    def test_successful_master_dry_run(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_mastering_deps", return_value=None
        ), patch("soundfile.read", return_value=(mock_data, 44100)), patch(
            "pyloudnorm.Meter"
        ) as mock_meter_cls:
    ...
    def test_settings_in_response(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            return_value=mock_master_result,
...
class TestFixDynamicTrack:
    ...
    def test_missing_deps(self):
        with patch.object(
            ...
            result = json.loads(
                _run(server.fix_dynamic_track("test-album", "01-test.wav"))
            )
        assert "error" in result
    ...
    def test_missing_track_file(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.fix_dynamic_track("test-album", "nonexistent.wav"))
            )
        assert "error" in result
        assert "not found" in result["error"]
...
class TestMasterWithReference:
    ...
    def test_missing_matchering(self):
        with patch.object(
            _processing_helpers,
            "_check_matchering",
            return_value="matchering not installed",
        ):
            result = json.loads(
    ...
    def _patch_ref_master(self):
        ...
        return patch.dict(
            "sys.modules", {"tools.mastering.reference_master": mock_mod}
        ), mock_fn
...
class TestTranscribeAudio:
    ...
    def test_missing_anthemscore(self):
        with patch.object(
            _processing_helpers,
            "_check_anthemscore",
            return_value="AnthemScore not found",
        ):
    ...
    def test_no_wav_files(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            _processing_helpers, "_import_sheet_music_module", return_value=mock_mod
        ):
            result = json.loads(_run(server.transcribe_audio("test-album")))
        assert "error" in result
        assert "No WAV" in result["error"]
...
class TestPrepareSingles:
    ...
    def test_successful_prepare(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_import_sheet_music_module", side_effect=_mock_import
        ):
            result = json.loads(_run(server.prepare_singles("test-album")))
        assert "tracks" in result
        assert result["track_count"] == 1
...
class TestCreateSongbook:
    ...
    def test_missing_deps(self):
        with patch.object(
            _processing_helpers, "_check_songbook_deps", return_value="Missing pypdf"
        ):
            result = json.loads(
                _run(server.create_songbook("test-album", "My Songbook"))
            )
        assert "error" in result
    ...
    def test_missing_audio_dir(self):
        ...
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_songbook_deps", return_value=None
        ):
            result = json.loads(
                _run(server.create_songbook("test-album", "My Songbook"))
            )
...
class TestGeneratePromoVideos:
    ...
    def test_missing_ffmpeg(self):
        with patch.object(
            _processing_helpers, "_check_ffmpeg", return_value="ffmpeg not found"
        ):
    ...
    def test_single_track_uses_markdown_title(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.promotion.generate_promo_video.generate_waveform_video",
...
class TestGenerateAlbumSampler:
    ...
    def test_sampler_failure(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.promotion.generate_album_sampler.generate_album_sampler",
            return_value=False,
        ):
            result = json.loads(_run(server.generate_album_sampler("test-album")))
        assert "error" in result
...
class TestMasterAudioComprehensive:
    ...
    def test_genre_preset_applied(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.mastering.master_tracks.load_genre_presets", return_value=presets
        ):
            result = json.loads(
                _run(server.master_audio("test-album", genre="hip-hop"))
            )
    ...
    def test_eq_settings_propagated(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            json.loads(
                _run(
                    server.master_audio("test-album", cut_highmid=-2.0, cut_highs=-1.5)
                )
            )
...
class TestFixDynamicTrackComprehensive:
    ...
    def test_successful_fix(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_mastering_deps", return_value=None
        ), patch("soundfile.read", return_value=(mock_data.copy(), 44100)), patch(
            "soundfile.write"
        ) as mock_write, patch("pyloudnorm.Meter") as mock_meter_cls:
            mock_meter = MagicMock()
            mock_meter.integrated_loudness.side_effect = [-22.0, -16.0, -14.0]
            mock_meter_cls.return_value = mock_meter

            result = json.loads(
                _run(server.fix_dynamic_track("test-album", "05-loud-track.wav"))
            )
...
class TestTranscribeAudioComprehensive:
    ...
    def test_single_track_success(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(
                    server.transcribe_audio("test-album", track_filename="01-track.wav")
                )
            )
    ...
    def test_format_param_parsed(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.transcribe_audio("test-album", formats="pdf,xml,midi"))
            )
    ...
    def test_dry_run_mode(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.transcribe_audio("test-album", dry_run=True))
            )
...
class TestPrepareSinglesComprehensive:
    ...
    def test_batch_multiple_tracks(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_import_sheet_music_module", side_effect=mock_import
        ):
            result = json.loads(_run(server.prepare_singles("test-album")))
    ...
    def test_xml_only_skips_pdf(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.prepare_singles("test-album", xml_only=True))
            )
...
class TestCreateSongbookComprehensive:
    ...
    def test_output_path_sanitized(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.create_songbook("test-album", "My Album / Songbook"))
            )
...
class TestGeneratePromoVideosComprehensive:
    ...
    def test_batch_mode_success(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            "tools.promotion.generate_promo_video.batch_process_album",
            side_effect=mock_batch,
        ), patch("tools.promotion.generate_promo_video.generate_waveform_video"), patch(
            "tools.shared.fonts.find_font", return_value="/fake/font.ttf"
        ):
            result = json.loads(_run(server.generate_promo_videos("test-album")))
...
class TestGenerateAlbumSamplerComprehensive:
    ...
    def test_custom_clip_duration(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.generate_album_sampler("test-album", clip_duration=8))
            )
    ...
    def test_custom_crossfade(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.generate_album_sampler("test-album", crossfade=1.0))
            )
    ...
    def test_twitter_limit_ok_true(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.generate_album_sampler("test-album", clip_duration=10))
            )
    ...
    def test_twitter_limit_ok_false(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.generate_album_sampler("test-album", clip_duration=12))
            )
...
class TestExtractTrackNumberFromStem:
    ...
    def test_double_digit(self):
        assert (
            _processing_helpers._extract_track_number_from_stem("12-beyond-the-stars")
            == 12
        )
...
class TestTranscribeAudioFlow:
    ...
    def test_dry_run_reports_title_mapping(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            _processing_helpers, "_resolve_audio_dir", return_value=(None, audio_dir)
        ), patch.object(
            _processing_helpers, "_import_sheet_music_module"
        ) as mock_import:
            mock_mod = MagicMock()
            mock_mod.find_anthemscore.return_value = "/usr/bin/anthemscore"
    ...
    def test_temp_dir_cleaned_up_on_failure(self, tmp_path):
        ...
        temp_dirs = glob_mod.glob(
            str(Path(tempfile.gettempdir()) / "test-album-transcribe-*")
        )
...
class TestPublishSheetMusic:
    ...
    def test_missing_audio_dir(self):
        ...
        mock_cache = MockStateCache(state)
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            _processing_helpers, "_check_cloud_enabled", return_value=None
        ):
            result = json.loads(_run(server.publish_sheet_music("test-album")))
        assert "error" in result
    ...
    def test_dry_run_lists_files(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            result = json.loads(
                _run(server.publish_sheet_music("test-album", dry_run=True))
            )
    ...
    def test_uploads_singles_and_songbook(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache), patch.object(
            ...
            _processing_helpers, "_import_cloud_module", return_value=mock_cloud_mod
    ...
    def test_persists_urls_to_track_frontmatter(self, tmp_path):
        ...
        state, mock_cloud_mod, mock_config, content_dir, _ = self._setup_publish_env(
            tmp_path
        )
        mock_cache = MockStateCache(state)
...
class TestUpdateFrontmatterBlock:
    ...
    def test_preserves_other_frontmatter(self, tmp_path):
        ...
        ok, _ = server._update_frontmatter_block(md, "sheet_music", {"pdf": "url"})
        assert ok is True
