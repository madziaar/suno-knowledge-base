...
class TestNgramsFromLines:
    ...
    def test_default_range_produces_2_3_4_grams(self):
        lines = [["burning", "shadows", "fall", "tonight"]]
        result = _text_analysis_mod._ngrams_from_lines(
            lines
        )  # default min_n=2, max_n=4
...
class TestCheckCrossTrackRepetition:
    ...
    def test_album_not_found(self):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_cross_track_repetition("nonexistent"))
            )
    ...
    def test_single_track_below_threshold(self, tmp_path):
        ...
        mock_cache = _build_state_with_tracks(
            tmp_path,
            {
    ...
    def test_custom_min_tracks_lowers_threshold(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_cross_track_repetition("test-album", min_tracks=2))
            )
    ...
    def test_min_tracks_floor_at_two(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_cross_track_repetition("test-album", min_tracks=1))
            )
        assert result["min_tracks_threshold"] == 2
    ...
    def test_min_tracks_zero_floors_to_two(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_cross_track_repetition("test-album", min_tracks=0))
            )
        assert result["min_tracks_threshold"] == 2
    ...
    def test_min_tracks_negative_floors_to_two(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_cross_track_repetition("test-album", min_tracks=-5))
            )
        assert result["min_tracks_threshold"] == 2
    ...
    def test_high_min_tracks_filters_everything(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(server.check_cross_track_repetition("test-album", min_tracks=10))
            )
        assert result["repeated_words"] == []
        assert result["repeated_phrases"] == []
    ...
    def test_phrase_entry_structure(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(_run(server.check_cross_track_repetition("test-album")))
        phrases = [
            p for p in result["repeated_phrases"] if p["phrase"] == "burning shadows"
        ]
        assert len(phrases) == 1
        phrase_entry = phrases[0]
        assert set(phrase_entry.keys()) == {
            "phrase",
            "track_count",
            "tracks",
            "total_occurrences",
    ...
    def test_summary_only_omits_arrays(self, tmp_path):
        ...
        with patch.object(_shared_mod, "cache", mock_cache):
            result = json.loads(
                _run(
                    server.check_cross_track_repetition(
                        "test-album",
                        summary_only=True,
                    )
                )
    ...
    def test_summary_only_counts_accurate(self, tmp_path):
        ...
        assert (
            summary["summary"]["flagged_phrases"] == full["summary"]["flagged_phrases"]
        )
    ...
    def test_max_results_truncates(self, tmp_path):
        ...
        assert (
            result["summary"]["flagged_phrases"] == full["summary"]["flagged_phrases"]
        )
...
class TestTokenizeLyricsWithSections:
    ...
    def test_only_section_tags(self):
        assert (
            _lyrics_analysis_mod._tokenize_lyrics_with_sections("[Verse]\n[Chorus]")
            == []
        )
...
class TestExtractDistinctiveNgrams:
        ...
        lines = [
            {
                ...
                "section_type": "chorus",
                "line_number": 1,
                "raw_line": "Burning shadows fall tonight",
            }
        ]
    ...
    def test_min_n_enforced(self):
        ...
        lines = [
            {
                "words": ["burning", "shadows", "fall"],
                "section": "Verse",
                "section_type": "verse",
                "line_number": 1,
                "raw_line": "Burning shadows fall",
            }
        ]
        result = _lyrics_analysis_mod._extract_distinctive_ngrams(lines)
    ...
    def test_common_phrases_filtered(self):
        ...
        lines = [
            {
                "words": ["middle", "of", "the", "night"],
                "section": "Chorus",
                "section_type": "chorus",
                "line_number": 1,
                "raw_line": "Middle of the night",
            }
        ]
        result = _lyrics_analysis_mod._extract_distinctive_ngrams(lines)
        phrases = [r["phrase"] for r in result]
    ...
    def test_stopword_only_ngrams_filtered(self):
        ...
        lines = [
            {
                "words": ["the", "and", "is", "but", "with"],
                "section": "Verse",
                "section_type": "verse",
                "line_number": 1,
                "raw_line": "The and is but with",
            }
        ]
        result = _lyrics_analysis_mod._extract_distinctive_ngrams(lines)
        assert len(result) == 0
    ...
    def test_custom_min_max_n(self):
        ...
        result = _lyrics_analysis_mod._extract_distinctive_ngrams(
            lines, min_n=5, max_n=5
        )
        for r in result:
...
class TestExtractDistinctivePhrases:
    ...
    def test_search_suggestions_capped_at_15(self):
        ...
        for i in range(20):
            lines.append(
                f"unique{i} alpha{i} beta{i} gamma{i} delta{i} epsilon{i} zeta{i}"
            )
        lyrics = "\n".join(lines)
        result = json.loads(_run(server.extract_distinctive_phrases(lyrics)))
    ...
    def test_chorus_priority_higher_than_verse(self):
        ...
        result = json.loads(_run(server.extract_distinctive_phrases(lyrics)))
        verse_priorities = [
            p["priority"] for p in result["phrases"] if p["section"] == "Verse 1"
        ]
        chorus_priorities = [
            p["priority"] for p in result["phrases"] if p["section"] == "Chorus"
        ]
    ...
    def test_max_phrases_zero_returns_all(self):
        ...
        result = json.loads(
            _run(server.extract_distinctive_phrases(lyrics, max_phrases=0))
        )
        assert result["total_phrases"] == len(result["phrases"])
        assert result["truncated"] is False
    ...
    def test_max_phrases_truncates(self):
        ...
        result = json.loads(
            _run(server.extract_distinctive_phrases(lyrics, max_phrases=3))
        )
    ...
    def test_max_phrases_exact_match_not_truncated(self):
        ...
        result = json.loads(
            _run(server.extract_distinctive_phrases(lyrics, max_phrases=n))
        )
    ...
    def test_max_phrases_larger_than_total(self):
        ...
        full = json.loads(_run(server.extract_distinctive_phrases(lyrics)))
        result = json.loads(
            _run(server.extract_distinctive_phrases(lyrics, max_phrases=9999))
        )
    ...
    def test_max_phrases_preserves_search_suggestions(self):
        ...
        lyrics = "\n".join(lines)
        result = json.loads(
            _run(server.extract_distinctive_phrases(lyrics, max_phrases=2))
        )
    ...
    def test_include_raw_lines_false_omits_field(self):
        ...
        result = json.loads(
            _run(
                server.extract_distinctive_phrases(
                    lyrics,
...
class TestGetRhymeTail:
    ...
    def test_rhyme_pair_match(self):
        ...
        assert _lyrics_analysis_mod._get_rhyme_tail(
            "away"
        ) == _lyrics_analysis_mod._get_rhyme_tail("day")
        assert _lyrics_analysis_mod._get_rhyme_tail(
            "shore"
        ) == _lyrics_analysis_mod._get_rhyme_tail("more")
        assert _lyrics_analysis_mod._get_rhyme_tail(
            "fire"
        ) == _lyrics_analysis_mod._get_rhyme_tail("desire")
...
class TestGetRhymeTailEdgeCases:
    ...
    def test_uppercase_normalization(self):
        assert _lyrics_analysis_mod._get_rhyme_tail(
            "NIGHT"
        ) == _lyrics_analysis_mod._get_rhyme_tail("night")
