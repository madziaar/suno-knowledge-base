...
def content_dir(tmp_path):
    ...
    (overrides_dir / "CLAUDE.md").write_text(
...
class TestToolsWithRealState:
    ...
    def test_get_track(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_track("integration-test-album", "01-first-track"))
        )
        assert result["found"] is True
    ...
    def test_format_for_clipboard(self, integration_env):
        ...
        result = json.loads(
            _run(server.format_for_clipboard("integration-test-album", "01", "lyrics"))
        )
        assert result["found"] is True
        assert "Testing the pipeline" in result["content"]
    ...
    def test_validate_album_structure(self, integration_env):
        ...
        result = json.loads(
            _run(server.validate_album_structure("integration-test-album"))
        )
        assert result["found"] is True
    ...
    def test_extract_links_from_sources(self, integration_env):
        ...
        result = json.loads(
            _run(server.extract_links("integration-test-album", "SOURCES.md"))
        )
        assert result["found"] is True
    ...
    def test_extract_links_from_track(self, integration_env):
        ...
        result = json.loads(
            _run(server.extract_links("integration-test-album", "02-second-track"))
        )
        assert result["found"] is True
    ...
    def test_get_lyrics_stats(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_lyrics_stats("integration-test-album", "01"))
        )
        assert result["found"] is True
    ...
    def test_run_pre_generation_gates(self, integration_env):
        ...
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(server.run_pre_generation_gates("integration-test-album", "01"))
            )
        assert result["found"] is True
        track = result["tracks"][0]
...
class TestUpdateTrackFieldEndToEnd:
    ...
    def test_state_reflects_file_update(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_track("integration-test-album", "02-second-track"))
        )
...
class TestRemainingToolsCoverage:
    ...
    def test_resolve_path_content(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_path("content", "integration-test-album"))
        )
        expected = str(
            integration_env["content_root"]
            / "artists"
            / "test-artist"
            / "albums"
            / "electronic"
    ...
    def test_resolve_path_audio(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_path("audio", "integration-test-album"))
        )
        expected = str(
            integration_env["audio_root"]
            / "artists"
            / "test-artist"
            / "albums"
            / "electronic"
    ...
    def test_resolve_path_tracks(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_path("tracks", "integration-test-album"))
        )
    ...
    def test_resolve_track_file(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_track_file("integration-test-album", "01-first-track"))
        )
        assert result["found"] is True
        assert result["track_slug"] == "01-first-track"
    ...
    def test_resolve_track_file_prefix(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_track_file("integration-test-album", "02"))
        )
        assert result["found"] is True
    ...
    def test_list_track_files_with_filter(self, integration_env):
        ...
        result = json.loads(
            _run(
                server.list_track_files("integration-test-album", status_filter="Final")
            )
        )
    ...
    def test_scan_artist_names_clean(self, integration_env, monkeypatch):
        ...
        result = json.loads(
            _run(server.scan_artist_names("electronic synth-driven ambient pads"))
        )
    ...
    def test_scan_artist_names_finds_match(self, integration_env, monkeypatch):
        ...
        if blocklist:
            ...
            result = json.loads(
                _run(server.scan_artist_names(f"This sounds like {artist_name} style"))
            )
    ...
    def test_check_explicit_content_respects_overrides(
        ...
        result = json.loads(
            _run(server.check_explicit_content("What the heck is happening"))
        )
        assert result["has_explicit"] is True
        found_words = [f["word"] for f in result["matches"]]
    ...
    def test_create_album_structure(self, integration_env):
        ...
        result = json.loads(
            _run(server.create_album_structure("new-test-album", "hip-hop"))
        )
    ...
    def test_create_album_structure_already_exists(self, integration_env):
        ...
        result = json.loads(
            _run(server.create_album_structure("integration-test-album", "electronic"))
        )
...
class TestGetTrackExtended:
    ...
    def test_track_not_found(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_track("integration-test-album", "99-missing"))
        )
        assert result["found"] is False
...
class TestExtractSectionExtended:
    ...
    def test_prefix_match(self, integration_env):
        ...
        result = json.loads(
            _run(server.extract_section("integration-test-album", "02", "lyrics"))
        )
        assert result["found"] is True
        assert result["track_slug"] == "02-second-track"
...
class TestFormatForClipboardExtended:
    ...
    def test_style_content(self, integration_env):
        ...
        result = json.loads(
            _run(server.format_for_clipboard("integration-test-album", "01", "style"))
        )
        assert result["found"] is True
        assert "electronic" in result["content"]
        assert "120 BPM" in result["content"]
    ...
    def test_streaming_content(self, integration_env):
        ...
        result = json.loads(
            _run(
                server.format_for_clipboard("integration-test-album", "01", "streaming")
            )
        )
        assert result["found"] is True
        assert "Testing the pipeline" in result["content"]
    ...
    def test_all_content(self, integration_env):
        ...
        result = json.loads(
            _run(server.format_for_clipboard("integration-test-album", "01", "all"))
        )
        assert result["found"] is True
...
class TestExtractLinksExtended:
    ...
    def test_file_not_found(self, integration_env):
        ...
        result = json.loads(
            _run(server.extract_links("integration-test-album", "MISSING.md"))
        )
        assert result["found"] is False
...
class TestCheckHomographsExtended:
    ...
    def test_multiple_homographs(self, integration_env):
        ...
        result = json.loads(
            _run(server.check_homographs("Live close to the wind, read the lead"))
        )
...
class TestRunPreGenerationGatesExtended:
    ...
    def test_track_02_has_blocking_gates(self, integration_env):
        ...
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(server.run_pre_generation_gates("integration-test-album", "02"))
            )
        track = result["tracks"][0]
    ...
    def test_all_tracks(self, integration_env):
        ...
        with patch.object(_text_analysis_mod, "_artist_blocklist_cache", None):
            result = json.loads(
                _run(server.run_pre_generation_gates("integration-test-album"))
            )
        assert result["found"] is True
        assert len(result["tracks"]) == 3
...
class TestRebuildStateExtended:
    ...
    def test_detects_new_album(self, integration_env):
        ...
        (new_album / "README.md").write_text(
            ALBUM_README.replace("Integration Test Album", "Brand New Album").replace(
                "integration-test-album", "brand-new-album"
            )
        )
        result = json.loads(_run(server.rebuild_state()))
    ...
    def test_after_track_addition(self, integration_env):
        ...
        new_track.write_text(
            TRACK_01.replace("First Track", "Fourth Track").replace("01", "04")
        )
        result = json.loads(_run(server.rebuild_state()))
...
class TestResolvePathExtended:
    ...
    def test_documents_path(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_path("documents", "integration-test-album"))
        )
        assert (
            "/artists/test-artist/albums/electronic/integration-test-album"
            in result["path"]
        )
        assert result["genre"] == "electronic"
...
class TestResolveTrackFileExtended:
    ...
    def test_track_not_found(self, integration_env):
        ...
        result = json.loads(
            _run(server.resolve_track_file("integration-test-album", "99-missing"))
        )
        assert result["found"] is False
...
class TestCreateAlbumStructureExtended:
    ...
    def test_genre_slug_normalization(self, integration_env):
        ...
        result = json.loads(
            _run(server.create_album_structure("slug-test-album", "Hip Hop"))
        )
        assert result["created"] is True
    ...
    def test_path_includes_artist(self, integration_env):
        ...
        result = json.loads(
            _run(server.create_album_structure("artist-check-album", "rock"))
        )
...
class TestListSkillsIntegration:
    ...
    def test_no_match(self, integration_env):
        ...
        result = json.loads(
            _run(server.list_skills(model_filter="opus", category="nonexistent"))
        )
        assert result["count"] == 0
...
class TestWorkflowAlbumLifecycle:
    ...
    def test_create_album_then_query(self, integration_env):
        ...
        result = json.loads(
            _run(server.create_album_structure("lifecycle-album", "electronic"))
        )
        assert result["created"] is True
    ...
    def test_session_tracks_workflow(self, integration_env):
        ...
        _run(
            server.update_session(
                album="integration-test-album", track="01-first-track", phase="Writing"
            )
        )
        ...
        _run(server.update_session(track="02-second-track", phase="Generation"))
...
class TestWorkflowSourceVerification:
    ...
    def test_verify_sources_then_clear_pending(self, integration_env):
        ...
        if "integration-test-album" in pending:
            track_slugs = [
                t["slug"] for t in pending["integration-test-album"]["tracks"]
            ]
...
class TestWorkflowMultiFieldAtomicity:
    ...
    def test_sequential_field_updates_preserve_all(self, integration_env):
        ...
        json.loads(_run(server.get_track("integration-test-album", "03-third-track")))
        # The file should have both values updated
        resolve = json.loads(
            _run(server.resolve_track_file("integration-test-album", "03-third-track"))
        )
...
""")
        ...
        result = json.loads(
            _run(server.get_track("integration-test-album", "04-new-track"))
        )
        assert result["found"] is True
    ...
    def test_deleted_track_file_detected_after_rebuild(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_track("integration-test-album", "03-third-track"))
        )
        assert result["found"] is False
...
class TestUpdateAlbumStatusIntegration:
    ...
    def test_status_round_trip(self, integration_env):
        ...
        _run(server.update_album_status("integration-test-album", "Complete"))
        _run(
            server.update_album_status("integration-test-album", "Released", force=True)
        )
    ...
    def test_invalid_status_rejected(self, integration_env):
        ...
        result = json.loads(
            _run(server.update_album_status("integration-test-album", "BadStatus"))
        )
        assert "error" in result
...
class TestCreateTrackIntegration:
    ...
    def test_creates_track_from_template(self, integration_env, monkeypatch):
        ...
        result = json.loads(
            _run(server.create_track("integration-test-album", "04", "Brand New Track"))
        )
        assert result["created"] is True
    ...
    def test_reject_duplicate_track(self, integration_env, monkeypatch):
        ...
        # First call succeeds
        r1 = json.loads(
            _run(server.create_track("integration-test-album", "04", "Track"))
        )
...
class TestPromoIntegration:
    ...
    def test_promo_content_returns_file_text(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_promo_content("integration-test-album", "campaign"))
        )
        assert result["found"] is True
    ...
    def test_promo_content_missing_file(self, integration_env):
        ...
        result = json.loads(
            _run(server.get_promo_content("integration-test-album", "facebook"))
        )
        assert result["found"] is False
...
class TestNewToolWorkflows:
    ...
    def test_create_track_then_update_status(self, integration_env, monkeypatch):
        ...
        # Verify in state
        track = json.loads(
            _run(server.get_track("integration-test-album", "04-workflow-track"))
        )
