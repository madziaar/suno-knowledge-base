...
def _make_rows(
    n: int, album_slug: str = "test-album", platform: str = "twitter"
) -> list[dict]:
...
class _MultiPatch:
    ...
    def __enter__(self) -> None:
        ...
        p2 = patch(
            "handlers.database._get_db_connection", return_value=(self._fake_conn, None)
        )
...
class TestDbSearchTweetsPagination:
    ...
    def test_offset_skips_rows(self):
        ...
        with _patch_db(rows):
            result = json.loads(
                _run(server.db_search_tweets("album", limit=0, offset=5))
            )
        assert result["total"] == 10
        assert result["offset"] == 5
        assert len(result["tweets"]) == 5
    ...
    def test_offset_and_limit_combined(self):
        ...
        with _patch_db(rows):
            result = json.loads(
                _run(server.db_search_tweets("album", limit=3, offset=2))
            )
        assert result["total"] == 10
        assert len(result["tweets"]) == 3
        assert result["has_more"] is True
    ...
    def test_offset_near_end(self):
        ...
        with _patch_db(rows):
            result = json.loads(
                _run(server.db_search_tweets("album", limit=5, offset=8))
            )
        assert result["total"] == 10
        assert len(result["tweets"]) == 2
        assert result["has_more"] is False
    ...
    def test_offset_beyond_total(self):
        ...
        with _patch_db(rows):
            result = json.loads(
                _run(server.db_search_tweets("album", limit=50, offset=100))
            )
        assert result["total"] == 5
        assert result["tweets"] == []
        assert result["has_more"] is False
