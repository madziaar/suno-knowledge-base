...
class TestRenameAlbumSuccess:
    ...
    def test_derives_title_from_new_slug_when_no_title_given(self, cache_with_album):
        result = json.loads(
            _run(_rename_mod.rename_album("original-album", "new-album-name"))
        )
    ...
    def test_uses_explicit_new_title_when_given(self, cache_with_album):
        result = json.loads(
            _run(
                _rename_mod.rename_album(
                    "original-album", "new-slug", new_title="Completely Different"
                )
            )
        )
    ...
    def test_updates_readme_h1_heading(self, cache_with_album):
        ...
        _run(
            _rename_mod.rename_album(
                "original-album", "new-album", new_title="Shiny New Album"
            )
        )
...
class TestRenameAlbumErrors:
    def test_missing_album_returns_error(self, cache_with_album):
        result = json.loads(
            _run(_rename_mod.rename_album("does-not-exist", "new-album"))
        )
    ...
    def test_new_slug_collision_returns_error(self, cache_with_album):
        ...
        result = json.loads(
            _run(_rename_mod.rename_album("original-album", "existing-album"))
        )
    ...
    def test_same_slug_after_normalization_returns_error(self, cache_with_album):
        ...
        result = json.loads(
            _run(_rename_mod.rename_album("original-album", "Original-Album"))
        )
...
class TestRenameAlbumPathTraversal:
    def test_rejects_slug_with_slash(self, cache_with_album):
        result = json.loads(
            _run(_rename_mod.rename_album("original-album", "evil/slug"))
        )
        assert "error" in result

    def test_rejects_slug_with_parent_traversal(self, cache_with_album):
        result = json.loads(_run(_rename_mod.rename_album("original-album", "..")))
        assert "error" in result
    ...
    def test_rejects_null_byte(self, cache_with_album):
        result = json.loads(
            _run(_rename_mod.rename_album("original-album", "evil\x00slug"))
        )
        assert "error" in result
...
class TestRenameTrackSuccess:
    ...
    def test_zero_padded_prefix_match_works(self, cache_with_album):
        ...
        result = json.loads(
            _run(
                _rename_mod.rename_track("original-album", "01", "01-matched-by-prefix")
            )
        )
    ...
    def test_state_cache_is_updated(self, cache_with_album):
        ...
        _run(_rename_mod.rename_track("original-album", "01-first-track", "01-renamed"))
...
class TestRenameTrackErrors:
    ...
    def test_ambiguous_prefix_match_returns_error(self, cache_with_album):
        ...
        result = json.loads(
            _run(_rename_mod.rename_track("original-album", "01", "01-renamed"))
        )
...
class TestRenameTrackPathTraversal:
    def test_rejects_slug_with_slash(self, cache_with_album):
        result = json.loads(
            _run(
                _rename_mod.rename_track("original-album", "01-first-track", "01/evil")
            )
        )
        assert "error" in result
    ...
    def test_rejects_slug_with_parent_traversal(self, cache_with_album):
        result = json.loads(
            _run(_rename_mod.rename_track("original-album", "01-first-track", ".."))
        )
        assert "error" in result
