        ...
        result = json.loads(
            _run(
                _streaming_mod.update_streaming_url(
                    "test-album", "bandcamp", "https://example.com"
                )
            )
        )
...
"""
        with patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.read_text", return_value=readme_content
        ), patch("pathlib.Path.write_text"), patch(
            "tools.state.indexer.write_state"
        ), patch(
            "tools.state.parsers.parse_album_readme",
            return_value={"streaming_urls": {}},
        ):
            result = json.loads(
                _run(_streaming_mod.update_streaming_url("test-album", "spotify", ""))
            )
    ...
    def test_album_not_found(self):
        result = json.loads(
            _run(
                _streaming_mod.update_streaming_url(
                    "nonexistent", "spotify", "https://example.com"
                )
            )
        )
...
"""
    ...
    def test_no_frontmatter_returns_error(self):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.read_text", return_value="# No Frontmatter"
        ):
            result = json.loads(
                _run(
                    _streaming_mod.update_streaming_url(
...
"""
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "pathlib.Path.read_text", return_value=readme_content
        ), patch("pathlib.Path.write_text", side_effect=capture_write), patch(
            "tools.state.indexer.write_state"
        ), patch(
            "tools.state.parsers.parse_album_readme",
            return_value={"streaming_urls": {}},
...
class TestVerifyStreamingUrls:
    ...
    def test_all_empty_urls_returns_not_set(self):
        result = json.loads(
            _run(_streaming_mod.verify_streaming_urls("empty-urls-album"))
        )
        assert result["found"] is True
    ...
    def test_reachable_url_counted(self):
        ...
        with patch("urllib.request.urlopen", return_value=mock_resp):
            result = json.loads(
                _run(_streaming_mod.verify_streaming_urls("test-album"))
            )
        assert result["found"] is True
    ...
    def test_unsupported_url_scheme_rejected(self):
        ...
        result = json.loads(_run(_streaming_mod.verify_streaming_urls("test-album")))
