...
class TestReadStateRecovery:
    ...
    def test_corrupted_json_creates_backup(self, tmp_path: Path) -> None:
        ...
        with patch.object(indexer, "STATE_FILE", state_file), patch.object(
            indexer, "CACHE_DIR", tmp_path
        ):
            result = indexer.read_state()
    ...
    def test_corrupted_json_logs_error(
        ...
        assert any(
            "corrupt" in r.message.lower() or "backup" in r.message.lower()
