...
try:
    ...
    print(
        "     ~/.bitwize-music/venv/bin/pip install 'mcp[cli]>=1.2.0' pyyaml",
        file=sys.stderr,
    )
    print("", file=sys.stderr)
    print(
        "After installing, restart Claude Code to reload the plugin.", file=sys.stderr
    )
    print("=" * 70, file=sys.stderr)
    sys.exit(1)
...
class StateCache:
    ...
    def rebuild(self) -> dict[str, Any]:
        ...
        logger.info("State rebuilt: %d albums, %d tracks", album_count, track_count)
        return state
