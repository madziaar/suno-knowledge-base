...
async def _stage_analysis(ctx: MasterAlbumCtx) -> str | None:
    ...
    tinny_tracks = [
        r["filename"] for r in analysis_results if r["tinniness_ratio"] > 0.6
    ]
    ...
    ctx.dark_tracks = {
        r["filename"] for r in ctx.analysis_results if r.get("is_dark") is True
    }
    if ctx.dark_tracks:
        logger.info(
            "Analysis: %d dark track(s) — excluded from ADM tightening: %s",
            len(ctx.dark_tracks),
            sorted(ctx.dark_tracks),
        )
...
async def _stage_verification(ctx: MasterAlbumCtx) -> str | None:
    ...
    assert (
        ...
        and ctx.source_dir is not None
        and ctx.audio_dir is not None
    )
    ...
    if out_of_spec or album_range_fail:
        ...
        if recoverable:
            ...
            recovery_subtype = "PCM_24" if ctx.targets["output_bits"] > 16 else "PCM_16"
        ...
        if out_of_spec or album_range_fail:
            ...
            unrecoverable_map: dict[str, dict[str, Any]] = {
                r["filename"]: r for r in auto_recovered if not r.get("converged", True)
            }
            halt_eligible_tracks: list[dict[str, Any]] = [
                s for s in out_of_spec if s["filename"] not in unrecoverable_map
            ]
...
def _coherence_finalize_stage(
    *,
    ...
    clamp_bound = [
        c for c in unconvergent if c.get("reason") == _COHERENCE_REASON_CLAMP
    ]
...
async def _stage_coherence_correct(ctx: MasterAlbumCtx) -> str | None:
    ...
    frozen_anchor_lufs = (
        float(current_verify[anchor_idx - 1].get("lufs", 0.0))
        if 1 <= anchor_idx <= len(current_verify)
        else 0.0
    )
...
async def _stage_ceiling_guard(
    ...
    compute_fn = (
        _compute_overshoots
        if _compute_overshoots is not None
        else _ceiling_guard_compute_overshoots
    )

    assert ctx.output_dir is not None

    ceiling_tracks_input = [
        {"filename": r["filename"], "lufs": r["lufs"]} for r in ctx.verify_results
    ]
    ...
    ceiling_stage: dict[str, Any] = {
        ...
        "median_lufs": (
            None
            if ceiling_result["median_lufs"] is None
            else round(ceiling_result["median_lufs"], 2)
        ),
        "threshold_lu": (
            None
            if ceiling_result["threshold_lu"] is None
    ...
    if correctable_rows:
        ...
        filename_to_index = {r["filename"]: i for i, r in enumerate(ctx.verify_results)}
...
async def _stage_layout(ctx: MasterAlbumCtx) -> str | None:
    ...
    if layout_path.is_file():
        prior_transitions = _parse_layout_yaml(layout_path.read_text(encoding="utf-8"))
...
async def _stage_status_update(ctx: MasterAlbumCtx) -> str | None:
    ...
    if album_data:
        ...
        all_final = all(
            t.get("status", "").lower() == TRACK_FINAL.lower() for t in tracks.values()
        )
        ...
        signature_present = (
            ctx.audio_dir is not None and (ctx.audio_dir / SIGNATURE_FILENAME).is_file()
        )
        ...
        if ctx.frozen_signature is not None:
            ...
            if not _tolerances:
                _tolerances = {
                    ...
                    if ctx.preset_dict is not None
                    and ctx.preset_dict.get(k) is not None
                }
        else:
            _lra_target_lu = (
                ctx.preset_dict.get("genre_ideal_lra_lu") if ctx.preset_dict else None
            )
            _tolerances = {
                k: ctx.preset_dict.get(k)
                for k in (
        ...
        if ctx.frozen_signature is not None:
            ...
            payload: dict[str, Any] = {
                ...
                "pipeline": {
                    ...
                    "upsampled_from_source": bool(
                        ctx.targets.get("upsampled_from_source")
                    ),
                },
            }
        else:
