...
async def generate_promo_videos(
    ...
    if track_filename:
                ...
                return _safe_json(
                    {
                        ...
                        "available_files": [
                            f.name
                            for f in _find_wav_source_dir(audio_dir).glob("*.wav")
                        ],
                    }
                )
        ...
        results = [
            {"filename": f.name, "output": str(f), "success": True}
...
async def generate_album_sampler(
    ...
    if success and output_path.exists():
        ...
        track_count = sum(
            1
            for f in _find_wav_source_dir(audio_dir).iterdir()
            if f.suffix.lower() in audio_extensions
        )
        expected_duration = (
            track_count * clip_duration - max(0, track_count - 1) * crossfade
        )
