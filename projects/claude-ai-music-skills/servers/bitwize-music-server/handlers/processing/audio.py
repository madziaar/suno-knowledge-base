...
async def master_audio(
    ...
    for wav_file in wav_files:
        ...
        if dry_run:
            ...
            result = await loop.run_in_executor(
                None, _do_master, wav_file, output_path, fade_out_val
            )
    ...
    if not track_results:
        return _safe_json(
            {"error": "No tracks processed (all silent or no WAV files)."}
        )
        ...
        return _safe_json(
            {
                ...
                "available_files": [
                    f.name for f in _find_wav_source_dir(audio_dir).glob("*.wav")
                ],
            }
        )
    ...
    if target_filename:
        ...
        try:
            ...
            return _safe_json(
                {
                    "tracks": [
                        {"filename": target_filename, "success": False, "error": str(e)}
                    ],
        ...
        for wav_file in wav_files:
            ...
            try:
                ...
                results.append(
                    {"filename": wav_file.name, "success": False, "error": str(e)}
                )
...
async def master_album(
    ...
    **Progress visibility:** stage-level progress is written to
    ...
    ctx = _album_stages.MasterAlbumCtx(
        ...
        cut_highs=cut_highs,
        source_subfolder=source_subfolder,
        freeze_signature=freeze_signature,
    ...
    def _inject_notices_and_return(result: str) -> str:
        ...
        try:
            ...
            logger.warning(
                "master_album: halt result is not valid JSON, notices not injected: %s",
                exc,
            )
        return result
        ...
        for stage_fn in adm_loop_stages:
            if result := await _run_stage(stage_fn):
                try:
                    ...
                    _d = {}
                is_adm_clip_failure = _d.get(
                    "failed_stage"
                ) == "adm_validation" and _d.get("failure_detail", {}).get(
                    "clips_retry_eligible"
                )
                if is_adm_clip_failure:
                    adm_last_failure_detail = _d.get("failure_detail") or {}
                    clip_entries = (
                        adm_last_failure_detail.get(
                            "tracks_with_clips",
                        )
                        or []
                    )
                    ...
                    for fname in dark_clipping:
                        if fname not in dark_adm_casualties:
                            entry = next(
                                (e for e in clip_entries if e.get("filename") == fname),
                                None,
                            )
                    ...
                    for fname in sorted(tightenable):
                        ...
                        if new_ceiling >= current:
                            ...
                            per_track_decisions[fname] = {
                                ...
                                "reason": ("per-track ceiling at adaptive floor"),
                                "cycle_detected": adm_cycle,
                                "final_ceiling": current,
                                "peak_db_decoded": float(
                                    entry.get("peak_db_decoded", 0.0),
    ...
    if adm_clip_failure_persisted:
        ...
        if adm_tightening_cycles == 0:
            ...
            if dark_casualty_count:
                reason_text += f"; {dark_casualty_count} dark track(s) not tightened"
            warn_text = (
...
async def album_coherence_correct(
    ...
    if not _is_path_confined(audio_dir, source_subfolder) or not _is_path_confined(
        audio_dir, check_subfolder
    ):
        return _safe_json(
            {
    ...
    polished_names = {
        f.name
        for f in polished_dir.iterdir()
        if f.suffix.lower() == ".wav" and "venv" not in str(f)
    }
    mastered_names = {
        f.name
        for f in mastered_dir.iterdir()
        if f.suffix.lower() == ".wav" and "venv" not in str(f)
    }
    ...
    try:
        for entry in plan["corrections"]:
            ...
            applied_target = entry.get("corrected_target_lufs", plan["anchor_lufs"])
