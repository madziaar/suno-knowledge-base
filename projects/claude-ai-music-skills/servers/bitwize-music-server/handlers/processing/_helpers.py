...
def _import_sheet_music_module(module_name: str) -> Any:
    ...
    try:
        ...
        logger.warning(
            "Optional sheet-music module %r not available: %s", module_name, exc
        )
        return None
    return mod
...
def _check_cloud_enabled() -> str | None:
    ...
    try:
        ...
        return "Could not load config. Ensure ~/.bitwize-music/config.yaml exists."
    if not config:
...
def _check_anthemscore() -> str | None:
    ...
    if transcribe_mod is not None:
        try:
            ...
            logger.warning(
                "AnthemScore check failed, falling back to path search: %s", exc
            )
