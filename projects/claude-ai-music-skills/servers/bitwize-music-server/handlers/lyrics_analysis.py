...
async def extract_distinctive_phrases(
    ...
    sections_found = sorted({line_data["section"] for line_data in lines})
...
async def count_syllables(text: str) -> str:
    ...
    for line_num, line in enumerate(text.split("\n"), 1):
        ...
        if _SECTION_TAG_RE.match(stripped):
            ...
            if current_lines:
                avg = sum(line["syllable_count"] for line in current_lines) / len(
                    current_lines
                )
                sections.append(
                    {
...
async def analyze_readability(text: str) -> str:
    ...
    avg_word_length = round(sum(len(w) for w in all_words) / total_words, 1)
...
async def analyze_rhyme_scheme(text: str) -> str:
    ...
    for sec_name in section_order:
        ...
        for ew in end_words:
            ...
            if not assigned:
                label = (
                    chr(ord("A") + next_label) if next_label < 26 else f"Z{next_label}"
                )
                next_label += 1
