...
def _inject_concept_into_readme(
    ...
    block = f"\n\n## Concept\n\n_Promoted from idea **{idea_title}**:_\n\n> {concept}\n"
...
def _set_promoted_to_field(title: str, slug: str) -> bool:
    ...
    promoted_re = re.compile(r"^\*\*Promoted To\*\*\s*:\s*.+$", re.MULTILINE)
...
async def create_idea(
    ...
    if not ideas_path:
        return _safe_json(
            {"error": "Cannot resolve IDEAS.md path (no content_root in config)"}
        )
...
async def promote_idea(
    ...
    slug = (
        album_slug.strip()
        if album_slug.strip()
        else _derive_slug_from_title(idea_title)
    )
    if not slug:
        return _safe_json(
            {
