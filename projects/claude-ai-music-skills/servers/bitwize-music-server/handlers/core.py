...
async def get_ideas(status_filter: str = "") -> str:
    ...
    if status_filter:
        items = [
            i for i in items if i.get("status", "").lower() == status_filter.lower()
        ]
...
async def search(query: str, scope: str = "all") -> str:
    ...
    if scope in ("all", "tracks"):
        ...
        for album_slug, album in state.get("albums", {}).items():
            for track_slug, track in album.get("tracks", {}).items():
                ...
                if query_lower in track_slug.lower() or query_lower in title.lower():
    ...
    if scope in ("all", "ideas"):
        ...
        for idea in state.get("ideas", {}).get("items", []):
            ...
            if query_lower in title.lower() or query_lower in genre.lower():
    ...
    if scope in ("all", "skills"):
        ...
        for name, skill in state.get("skills", {}).get("items", {}).items():
            ...
            if (
                ...
                or query_lower in model_tier.lower()
            ):
...
async def resolve_path(path_type: str, album_slug: str, genre: str = "") -> str:
    ...
    base = (
        Path(root_map[path_type]) / "artists" / artist / "albums" / genre / normalized
    )
    if path_type == "tracks":
        base = base / "tracks"
...
async def resolve_track_file(album_slug: str, track_slug: str) -> str:
    ...
    prefix_matches = {
        slug: data for slug, data in tracks.items() if slug.startswith(normalized_track)
    }
...
async def update_track_field(
                ...
                return _safe_json(
                    {
                        ...
                        "failed_gates": [
                            g for g in gate_results if g.get("severity") == "BLOCKING"
                        ],
    ...
    try:
            ...
            tracks[matched_slug].update(
                {
                    ...
                    "explicit": parsed.get(
                        "explicit", tracks[matched_slug].get("explicit")
                    ),
                    "has_suno_link": parsed.get(
                        "has_suno_link", tracks[matched_slug].get("has_suno_link")
                    ),
                    "sources_verified": parsed.get(
                        "sources_verified", tracks[matched_slug].get("sources_verified")
                    ),
        ...
        logger.warning(
            "File written but cache update failed for %s.%s: %s",
            normalized_album,
            matched_slug,
            e,
        )
...
async def get_album_progress(album_slug: str) -> str:
    ...
    completion_pct = (
        round((tracks_completed / track_count * 100), 1) if track_count > 0 else 0.0
    )
