...
async def get_album_full(
    ...
    if not album:
        matches = {
            s: d for s, d in albums.items() if normalized in s or s in normalized
        }
        if len(matches) == 1:
...
async def validate_album_structure(
    ...
    genre = album.get("genre", "")
    audio_path = str(
        Path(audio_root) / "artists" / artist / "albums" / genre / normalized
    )
    ...
    if "audio" in check_set:
        ...
        if audio_p.is_dir():
            ...
            _fail(
                "audio",
                "Audio in wrong location (missing artist folder)",
    ...
    if "tracks" in check_set:
        ...
        for t_slug, t_data in sorted(tracks.items()):
            ...
            if track_issues:
                _warn(
                    "tracks",
                    f"{t_slug}: Status={status}, issues: {', '.join(track_issues)}",
                )
            else:
