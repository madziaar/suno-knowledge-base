...
async def check_homographs(text: str) -> str:
    ...
    return _safe_json(
        {"has_homographs": len(results) > 0, "matches": results, "count": len(results)}
    )


# Artist blocklist cache — loaded lazily from reference file
_artist_blocklist_cache: list[dict[str, str]] | None = None
_artist_blocklist_patterns: dict[str, re.Pattern[str]] | None = (
    None  # name -> compiled re.Pattern
)
...
def _load_artist_blocklist() -> list[dict[str, str]]:
    ...
    with _artist_blocklist_lock:
        ...
        blocklist_path = (
            _shared.PLUGIN_ROOT / "reference" / "suno" / "artist-blocklist.md"
        )
        try:
            current_mtime = (
                blocklist_path.stat().st_mtime if blocklist_path.exists() else 0.0
            )
        except OSError:
            current_mtime = 0.0
        if (
            _artist_blocklist_cache is not None
            and current_mtime == _artist_blocklist_mtime
        ):
...
async def check_pronunciation_enforcement(
    ...
    matched_slug, track_data, error = _find_track_or_error(
        tracks, track_slug, album_slug
    )
    if error:
        return error
    ...
    if not track_path:
        return _safe_json(
            {"found": False, "error": f"No path stored for track '{matched_slug}'"}
        )
    ...
    for entry in entries:
        ...
        occurrences = len(
            re.findall(re.escape(phonetic), lyrics_content, re.IGNORECASE)
        )
...
_explicit_word_patterns: dict[str, re.Pattern[str]] | None = (
    None  # word -> compiled re.Pattern
)
    ...
    with _explicit_word_lock:
        ...
        try:
            if override_path and override_path.exists():
                ...
                add_section = _extract_markdown_section(
                    text, "Additional Explicit Words"
                )
                if add_section:
                ...
                remove_section = _extract_markdown_section(
                    text, "Not Explicit (Override Base)"
                )
                if remove_section:
...
async def extract_links(
    ...
    if not track:
        # Try prefix match
        prefix_matches = {
            s: d for s, d in tracks.items() if s.startswith(normalized_file)
        }
...
def _ngrams_from_lines(
    lines: list[list[str]], min_n: int = 2, max_n: int = 4
) -> list[str]:
...
async def get_lyrics_stats(
    ...
    if track_slug:
        matched_slug, track_data, error = _find_track_or_error(
            all_tracks, track_slug, album_slug
        )
        if error:
            return error
        assert track_data is not None
...
async def check_cross_track_repetition(
    ...
    if not summary_only:
        ...
        phrases_out = (
            repeated_phrases[:max_results] if max_results > 0 else repeated_phrases
        )
        result["repeated_words"] = words_out
        result["repeated_phrases"] = phrases_out
        result["truncated"] = max_results > 0 and (
            len(repeated_words) > max_results or len(repeated_phrases) > max_results
        )
