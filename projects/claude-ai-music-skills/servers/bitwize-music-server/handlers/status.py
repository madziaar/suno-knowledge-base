...
_VALID_TRACK_STATUSES = {
    TRACK_NOT_STARTED.lower(),
    TRACK_SOURCES_PENDING.lower(),
    TRACK_SOURCES_VERIFIED.lower(),
    TRACK_IN_PROGRESS.lower(),
    TRACK_GENERATED.lower(),
    TRACK_FINAL.lower(),
}
...
_TRACK_STATUS_LEVEL = {
    TRACK_NOT_STARTED: 0,
    TRACK_SOURCES_PENDING: 1,
    TRACK_SOURCES_VERIFIED: 2,
    TRACK_IN_PROGRESS: 3,
    TRACK_GENERATED: 4,
    TRACK_FINAL: 5,
}
_ALBUM_STATUS_LEVEL = {
    ALBUM_CONCEPT: 0,
    ALBUM_RESEARCH_COMPLETE: 1,
    ALBUM_SOURCES_VERIFIED: 2,
    ALBUM_IN_PROGRESS: 3,
    ALBUM_COMPLETE: 4,
    ALBUM_RELEASED: 5,
}
...
def _validate_track_transition(
    current: str, new: str, *, force: bool = False
) -> str | None:
...
def _validate_album_transition(
    current: str, new: str, *, force: bool = False
) -> str | None:
    """Return error message if transition is invalid, or None if OK."""
    if force:
...
def _check_album_track_consistency(
    album: dict[str, Any], new_status: str
) -> str | None:
    ...
    if canonical == ALBUM_IN_PROGRESS:
        ...
        below = [
            ...
            if _TRACK_STATUS_LEVEL.get(t.get("status", TRACK_NOT_STARTED), 0)
            < _TRACK_STATUS_LEVEL[TRACK_GENERATED]
        ]
        if below:
            return (
...
async def update_album_status(album_slug: str, status: str, force: bool = False) -> str:
    ...
    if not force:
        ...
        if require_source_path:
            canonical_status = _CANONICAL_ALBUM_STATUS.get(
                status.lower().strip(), status
            )
            canonical_current = _CANONICAL_ALBUM_STATUS.get(
                current_status.lower().strip(), current_status
            )
            if (
                canonical_current == ALBUM_CONCEPT
                and canonical_status == ALBUM_IN_PROGRESS
            ):
    ...
    if status.lower().strip() == ALBUM_SOURCES_VERIFIED.lower() and not force:
        ...
        unverified = [
            ...
            if t.get("status", TRACK_NOT_STARTED)
            in {TRACK_NOT_STARTED, TRACK_SOURCES_PENDING}
        ]
        if unverified:
            return _safe_json(
                {
    ...
    if canonical_status == ALBUM_RELEASED and not force:
        ...
        audio_path = (
            Path(audio_root) / "artists" / artist_name / "albums" / genre / normalized
        )
        if not audio_path.is_dir() or not list(
            _find_wav_source_dir(audio_path).glob("*.wav")
        ):
        ...
        explicit_tracks = [s for s, t in tracks.items() if t.get("explicit") is True]
        ...
        for t_slug, t_data in tracks.items():
            ...
            if any(m.lower() in block.lower() for m in _STREAMING_PLACEHOLDER_MARKERS):
                streaming_issues.append(
                    f"{t_slug}: placeholder content in Streaming Lyrics"
                )
        if streaming_issues:
            release_issues.append(
    ...
    try:
        ...
        logger.warning(
            "File written but cache update failed for album %s: %s", normalized, e
        )
...
async def create_track(
    ...
    content = content.replace(
        "[Album Name](../README.md)", f"[{album_title}](../README.md)"
    )
    ...
    if not documentary:
        ...
        if source_start != -1 and source_end != -1:
            content = (
                ...
                + content[source_end + len("<!-- END SOURCE SECTIONS -->") :]
            )
        ...
        if doc_start != -1 and doc_end != -1:
            content = (
                content[:doc_start]
                + content[doc_end + len("<!-- END DOCUMENTARY SECTIONS -->") :]
            )
