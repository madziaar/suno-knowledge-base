...
async def rename_album(old_slug: str, new_slug: str, new_title: str = "") -> str:
    ...
    if normalized_old == normalized_new:
        return _safe_json(
            {"error": "Old and new slugs are the same after normalization."}
        )
    ...
    # Resolve paths
    content_dir_old = (
        Path(content_root) / "artists" / artist / "albums" / genre / normalized_old
    )
    content_dir_new = (
        Path(content_root) / "artists" / artist / "albums" / genre / normalized_new
    )
    audio_dir_old = (
        Path(audio_root) / "artists" / artist / "albums" / genre / normalized_old
    )
    audio_dir_new = (
        Path(audio_root) / "artists" / artist / "albums" / genre / normalized_new
    )
    docs_dir_old = (
        Path(documents_root) / "artists" / artist / "albums" / genre / normalized_old
    )
    docs_dir_new = (
        Path(documents_root) / "artists" / artist / "albums" / genre / normalized_new
    )
    ...
    if readme_path.exists():
        try:
            ...
            if match:
                updated_text = (
                    text[: match.start()] + f"# {title}" + text[match.end() :]
                )
...
async def rename_track(
    ...
    matched_slug, track_data, error = _find_track_or_error(
        tracks, old_track_slug, album_slug
    )
    if error:
        return error
    ...
    try:
        if _normalize_slug(old_track_slug) == normalized_new:
            return _safe_json(
                {"error": "Old and new track slugs are the same after normalization."}
            )
    except ValueError as exc:
        return _safe_json({"error": str(exc)})
    ...
    if not _is_path_confined(old_path.parent, f"{normalized_new}.md"):
        return _safe_json(
            {"error": "Invalid new track slug: would escape tracks directory"}
        )
    ...
    try:
        ...
        if match:
            ...
            if h1_match:
                updated_text = (
                    updated_text[: h1_match.start()]
                    + f"# {title}"
                    + updated_text[h1_match.end() :]
                )
            atomic_write_text(new_path, updated_text)
        else:
            logger.warning(
                "Title field not found in track metadata table for %s", matched_slug
            )
    except OSError as e:
    ...
    try:
        ...
        try:
            ...
            old_track_data.update(
                {
                    ...
                    "has_suno_link": parsed.get(
                        "has_suno_link", old_track_data.get("has_suno_link")
                    ),
                    "sources_verified": parsed.get(
                        "sources_verified", old_track_data.get("sources_verified")
                    ),
            ...
            logger.warning(
                "Could not re-parse track after rename %s: %s", new_path, exc
            )
        tracks[normalized_new] = old_track_data
        state = (
            _shared.cache.get_state_ref()
        )  # same object that album/tracks reference into
        if state:
            write_state(state)
    except Exception as e:
    ...
    logger.info(
        "Renamed track '%s' to '%s' in album '%s'",
        matched_slug,
        normalized_new,
        normalized_album,
    )
