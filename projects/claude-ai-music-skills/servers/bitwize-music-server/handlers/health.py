...
def _check_skill_registration() -> dict[str, Any]:
    ...
    source_skills = {
        p.parent.name for p in (_shared.PLUGIN_ROOT / "skills").glob("*/SKILL.md")
    }
    ...
    cached_skills = {p.parent.name for p in (cache_dir / "skills").glob("*/SKILL.md")}
...
async def check_venv_health() -> str:
    ...
    if status == "stale":
        result["fix_command"] = f"~/.bitwize-music/venv/bin/pip install -r {req_path}"
...
async def health_check() -> str:
    ...
    if skills_status == "ok":
        ...
        if skills_raw.get("missing"):
            parts.append(
                f"{len(skills_raw['missing'])} missing: {', '.join(skills_raw['missing'])}"
            )
        if skills_raw.get("ghost"):
            parts.append(
                f"{len(skills_raw['ghost'])} ghost: {', '.join(skills_raw['ghost'])}"
            )
        checks.append(
            {
                "name": "skills",
...
def _check_config() -> dict[str, Any]:
    ...
    return {
        "name": "config",
        "status": "ok",
        "detail": "All required fields set, paths accessible",
    }
...
def _check_database() -> dict[str, Any]:
    ...
    if not db_config.get("enabled"):
        return {"name": "database", "status": "ok", "detail": "Not enabled (optional)"}
...
def _check_cloud() -> dict[str, Any]:
    ...
    if not cloud.get("enabled"):
        return {"name": "cloud", "status": "ok", "detail": "Not enabled (optional)"}
    ...
    if provider == "r2":
        ...
        missing = [
            f
            for f in ("account_id", "access_key_id", "secret_access_key", "bucket")
            if not r2.get(f)
        ]
        if missing:
            return {
        ...
        s3 = cloud.get("s3", {})
        missing = [
            f for f in ("access_key_id", "secret_access_key", "bucket") if not s3.get(f)
        ]
        if missing:
            return {
                "name": "cloud",
