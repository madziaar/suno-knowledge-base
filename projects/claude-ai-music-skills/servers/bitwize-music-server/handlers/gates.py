...
def _check_pre_gen_gates_for_track(
    ...
    if not lyrics_content or not lyrics_content.strip():
        ...
        gates.append(
            {"gate": "Lyrics Reviewed", "status": "PASS", "detail": "Lyrics populated"}
        )
    ...
    if file_text:
        ...
        if pron_entries and lyrics_content:
            ...
            for entry in pron_entries:
                if not re.search(
                    re.escape(entry["phonetic"]), lyrics_content, re.IGNORECASE
                ):
                    unapplied.append(entry["word"])
            if unapplied:
                gates.append(
                    {
                        "gate": "Pronunciation Resolved",
                        "status": "FAIL",
                        "severity": "BLOCKING",
    ...
    if lyrics_content and lyrics_content.strip():
        ...
        gates.append(
            {"gate": "Lyric Length", "status": "SKIP", "detail": "No lyrics to check"}
        )
    ...
    for t_slug, t_data in sorted(tracks_to_check.items()):
        ...
        if track_path:
            try:
                ...
                logger.warning(
                    "Cannot read track file for pre-gen gates %s: %s", track_path, e
                )
    ...
    for t_slug, t_data in sorted(tracks_to_check.items()):
        ...
        if track_path:
            try:
                ...
                logger.warning(
                    "Cannot read track file for streaming check %s: %s", track_path, e
                )
        ...
        if streaming_content and streaming_content.strip():
            checks.append(
                {"check": "Not Empty", "status": "PASS", "detail": "Content present"}
            )
        else:
            checks.append(
                {
        ...
        if streaming_content and streaming_content.strip():
            ...
            if tagged_lines:
                examples = ", ".join(
                    f"{tag} (line {ln})" for ln, tag in tagged_lines[:5]
                )
                if len(tagged_lines) > 5:
                    examples += f", ... ({len(tagged_lines)} total)"
                checks.append(
                    {
                        "check": "No Section Tags",
                        "status": "WARN",
                        "severity": "WARNING",
            ...
            if streaming_word_count < 20:
                ...
                checks.append(
                    {"check": "Word Count", "status": "PASS", "detail": detail}
                )
        else:
