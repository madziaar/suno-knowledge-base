...
def load_genre_presets() -> dict[str, dict[str, float]]:
    ...
    for genre in all_genre_names:
        ...
        merged = {**base, **over}
        presets[genre] = {
            key: float(merged.get(key, default)) for key, default in defaults.items()
        }
...
def apply_eq(data: Any, rate: int, freq: float, gain_db: float, q: float = 1.0) -> Any:
    ...
    if not (20 <= freq < nyquist):
        logger.warning(
            "EQ freq %.1f Hz out of valid range (20\u2013%.0f Hz), skipping",
            freq,
            nyquist,
        )
        return data
    ...
    if not np.all(np.abs(poles) < 1.0):
        logger.warning(
            "Unstable EQ filter at %.1f Hz (gain=%.1f dB, Q=%.2f), skipping",
            freq,
            gain_db,
            q,
        )
...
def apply_high_shelf(data: Any, rate: int, freq: float, gain_db: float) -> Any:
    ...
    if not (20 <= freq < nyquist):
        logger.warning(
            "High shelf freq %.1f Hz out of valid range (20\u2013%.0f Hz), skipping",
            freq,
            nyquist,
        )
        return data
    ...
    if not np.all(np.abs(poles) < 1.0):
        logger.warning(
            "Unstable high shelf filter at %.1f Hz (gain=%.1f dB), skipping",
            freq,
            gain_db,
        )
        return data
...
def apply_low_shelf(
    data: Any, rate: int, freq: float, gain_db: float, q: float = 0.707
) -> Any:
    ...
    if not (20 <= freq < nyquist):
        logger.warning(
            "Low shelf freq %.1f Hz out of valid range (20–%.0f Hz), skipping",
            freq,
            nyquist,
        )
        return data
    ...
    if not np.all(np.abs(poles) < 1.0):
        logger.warning(
            "Unstable low shelf filter at %.1f Hz (gain=%.1f dB), skipping",
            freq,
            gain_db,
        )
        return data
...
def apply_tilt_eq(data: Any, rate: int, tilt_db: float, pivot_hz: float = 650.0) -> Any:
...
def _design_linear_phase_eq(
    ...
    if not (20 <= freq < nyquist):
        logger.warning(
            "Linear-phase EQ freq %.1f Hz out of range (20–%.0f Hz), skipping",
            freq,
            nyquist,
        )
        return None
...
def apply_highpass(data: Any, rate: int, cutoff: int = 30) -> Any:
    ...
    if cutoff >= nyquist:
        logger.warning(
            "Highpass cutoff %d Hz >= Nyquist (%.0f Hz), skipping", cutoff, nyquist
        )
        return data
...
def apply_stereo_width(
    data: Any, rate: int, width: float = 1.0, bass_mono_freq: int = 0
) -> Any:
...
def apply_fade_out(
    data: Any, rate: int, duration: float = 5.0, curve: str = "exponential"
) -> Any:
...
def soft_clip(data: Any, threshold: float = 0.95) -> Any:
    ...
    result[above_thresh] = np.sign(data[above_thresh]) * (
        threshold
        + (1 - threshold)
        * np.tanh((np.abs(data[above_thresh]) - threshold) / (1 - threshold))
    )
    return result
...
def limit_peaks_lookahead(
    ...
    if lookahead_samples < n_samples:
        ...
        padded = np.concatenate(
            [smoothed, np.ones(lookahead_samples, dtype=np.float64)]
        )
        shifted = minimum_filter1d(padded, size=window, origin=-(window // 2))[
            :n_samples
        ]
    else:
...
def apply_multiband_compress(
    ...
    if low_crossover >= high_crossover:
        logger.warning(
            "Multiband crossovers invalid (low=%.0f >= high=%.0f), skipping",
            low_crossover,
            high_crossover,
        )
        return data
...
def apply_midside_eq(
    ...
    if low_gain != 0:
        ...
        if linear_phase:
            ...
            side_2d = apply_low_shelf(
                side_2d, rate, freq=low_freq, gain_db=low_gain, q=0.707
            )
...
def apply_tpdf_dither(data: Any, target_bits: int = 16, seed: int | None = None) -> Any:
    ...
    noise = rng.uniform(-0.5, 0.5, size=data.shape) + rng.uniform(
        -0.5, 0.5, size=data.shape
    )
    noise *= one_lsb
...
def master_track(
    ...
    if preset is not None:
        ...
        if p["cut_highmid"] != 0:
            eq_settings.append(
                (p["eq_highmid_freq"], p["cut_highmid"], p["eq_highmid_q"])
            )
        if p["cut_highs"] != 0:
    ...
    if eq_settings:
        for freq, gain_db, q in eq_settings:
            if use_linear_phase:
                data = apply_linear_phase_eq(
                    data, rate, freq, gain_db, q, filter_type="peaking"
                )
            else:
    ...
    if ms_low_gain != 0 or ms_high_gain != 0:
        data = apply_midside_eq(
            data,
            rate,
            low_gain=ms_low_gain,
            low_freq=p.get("midside_low_freq", 300.0),
            high_gain=ms_high_gain,
            high_freq=p.get("midside_high_freq", 8000.0),
    ...
    if target_lra > 0 and measured_lra is not None:
        if measured_lra < target_lra:
            logger.warning(
                "Audio already over-compressed (LRA %.1f LU below target %.1f LU). "
                "Skipping expansion to avoid artifacts.",
                measured_lra,
                target_lra,
            )
            ...
            for i in range(max_iterations):
                ...
                ratio_scale = current_ratio / max(compress_ratio, 1.0)
                logger.info(
                    "LRA %.1f LU exceeds target %.1f LU — retrying with ratio %.2f (iter %d)",
                    measured_lra,
                    target_lra,
                    current_ratio,
                    i + 1,
                ...
                if oversample > 1:
                    recomp_data = signal.resample_poly(
                        recomp_data, up=oversample, down=1, axis=0
                    )
                ...
                if oversample > 1:
                    recomp_data = signal.resample_poly(
                        recomp_data, up=1, down=oversample, axis=0
                    )
                if compress_mix < 1.0:
                    recomp_data = (
                        pre_compress_data.copy() * (1.0 - compress_mix)
                        + recomp_data * compress_mix
                    )
                data = recomp_data
            ...
            if measured_lra is not None and measured_lra > target_lra + lra_tolerance:
                logger.warning(
                    "LRA %.1f LU still exceeds target %.1f LU after %d iterations",
                    measured_lra,
                    target_lra,
                    max_iterations,
                )
            ...
            if not np.isfinite(current_lufs):
                logger.warning(
                    "Audio became silent after LRA targeting, skipping: %s", input_path
                )
                return {
                    "original_lufs": float("-inf"),
                    "final_lufs": float("-inf"),
    ...
    final_peak = (
        20 * np.log10(true_peak_linear) if true_peak_linear > 0 else float("-inf")
    )
...
Examples:
  ...
  python master_tracks.py /path/to/tracks --dry-run --genre rock
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Analyze only, do not write files"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show debug output"
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Show only warnings and errors"
    )
    parser.add_argument(
        "--compress-ratio",
        type=float,
    ...
    parser.add_argument(
        "--deess", action="store_true", default=None, help="Enable de-esser"
    )
    parser.add_argument(
        "--deess-freq",
        type=float,
    ...
    if preset["cut_highmid"] != 0:
        print(
            f"EQ: High-mid cut: {preset['cut_highmid']}dB at {preset['eq_highmid_freq']}Hz (Q={preset['eq_highmid_q']})"
        )
    if preset["cut_highs"] != 0:
        print(
            f"EQ: High shelf cut: {preset['cut_highs']}dB at {preset['eq_highs_freq']}Hz (Q={preset['eq_highs_q']})"
        )
    if preset["compress_ratio"] > 1.0:
        print(
            f"Compression: {preset['compress_ratio']}:1 (threshold={preset['compress_threshold']}dB, attack={preset['compress_attack']}ms, release={preset['compress_release']}ms)"
        )
    else:
    ...
    if album_consistency > 0 and wav_files:
        ...
        if len(source_lufs) >= 2:
            ...
            for name, src_lufs in source_lufs.items():
                ...
                adjusted_target = base_target - np.clip(
                    deviation, -half_spread, half_spread
                )
                ...
                if abs(adjusted_target - base_target) > 0.05:
                    print(
                        f"  {name[:34]}: target adjusted to {adjusted_target:.1f} LUFS "
    ...
    if workers == 1:
        ...
        for wav_file, output_path in tasks:
            ...
            print(
                f"{name:<35} {result['original_lufs']:>7.1f} {result['final_lufs']:>7.1f} "
        ...
        for idx in sorted(ordered_results):
            ...
            display = name[:34]
            print(
                f"{display:<35} {result['original_lufs']:>7.1f} {result['final_lufs']:>7.1f} "
                f"{result['gain_applied']:>+7.1f} {result['final_peak']:>7.1f}"
            )
