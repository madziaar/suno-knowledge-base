...
def _bandpass_sos(
    data: np.ndarray, rate: int, low_hz: float, high_hz: float, order: int = 4
) -> np.ndarray:
...
def analyze_track(
    filepath: Path | str, *, vocal_stem_path: Path | str | None = None
) -> dict[str, Any]:
    ...
    for band_name, (low, high) in bands.items():
        ...
        band_energy[band_name] = (
            (energy / total_energy) * 100 if total_energy > 0 else 0.0
        )

    # Tinniness indicator: ratio of high_mid to mid energy
    tinniness_ratio = (
        band_energy["high_mid"] / band_energy["mid"] if band_energy["mid"] > 0 else 0
    )
...
def main() -> None:
    ...
    for r in results:
        delta = -14 - r["lufs"]
        print(
            f"{r['filename'][:34]:<35} {r['lufs']:>8.1f} {r['peak_db']:>8.1f} {delta:>+10.1f}"
        )
    ...
    if has_dynamics:
        ...
        for r in results:
            ...
            print(
                f"{r['filename'][:34]:<35} {st_str:>8} {mom_str:>8} {st_range:>7.1f}dB"
            )
        print()
    ...
    print("=" * 80)
    print(
        f"{'Track':<25} {'Bass':>7} {'Mid':>7} {'HiMid':>7} {'High':>7} {'Tinny?':>8}"
    )
    print("-" * 65)
    ...
    for r in results:
        ...
        name = r["filename"][:24]
        print(
            f"{name:<25} {bass:>6.1f}% {mid:>6.1f}% {himid:>6.1f}% {high:>6.1f}% {tinny:>8}"
        )
