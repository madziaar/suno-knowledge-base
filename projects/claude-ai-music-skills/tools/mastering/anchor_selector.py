...
def _spectral_match_score(
    band_energy: dict[str, float], reference: dict[str, float]
) -> float:
...
def _mix_quality_score(
    track: dict[str, Any], spectral_reference: dict[str, float], genre_ideal_lra: float
) -> float:
...
def _representativeness_score(
    track: dict[str, Any], medians: dict[str, float | None]
) -> float:
...
def select_anchor(
    ...
    if (
        len(ranked) >= 2
        and abs(ranked[0]["score"] - ranked[1]["score"]) < TIE_BREAKER_EPSILON
    ):
        method = "tie_breaker"
