...
def _check_format(info: Any) -> dict[str, str]:
    ...
    if rate not in (44100, 48000, 88200, 96000, 176400, 192000):
        issues.append(f"Sample rate {rate} Hz, expected 44.1/48/88.2/96/176.4/192 kHz")
        status = "FAIL"
...
def _check_phase(data: Any, rate: int) -> dict[str, str]:
    ...
    if not correlations:
        return {
            "status": "PASS",
            "value": "N/A",
            "detail": "No significant audio to measure",
        }
...
def _check_clipping(data: Any) -> dict[str, str]:
    ...
    clipped = (
        np.any(np.abs(data) >= 0.99, axis=1) if data.ndim > 1 else np.abs(data) >= 0.99
    )
...
def _resolve_click_thresholds(genre: str | None) -> tuple[float, int]:
    ...
    return float(preset.get("click_peak_ratio", 6.0)), int(
        preset.get("click_fail_count", 3)
    )
