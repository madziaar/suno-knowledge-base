...
def _afconvert_encode_decode(input_path: Path) -> tuple[np.ndarray, int, str]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            raise ADMValidationError(
                f"ffmpeg decode after afconvert timed out after {_FFMPEG_TIMEOUT_SEC}s"
            ) from exc
        if dec.returncode != 0:
            raise ADMValidationError(
...
def render_adm_validation_markdown(
    ...
    *,
    ...
    for r in results:
        ...
        lines.append(f"| {fname} | {peak} | {r['clip_count']} | {verdict} |")
    ...
    if clips_total > 0:
        ...
        if dark_fail_count == fail_count:
            ...
            lines.append("> Tighten true-peak ceiling by 0.5 dB and re-master.")
    return "\n".join(lines) + "\n"
