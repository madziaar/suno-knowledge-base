...
def classify_outliers(
    ...
    for delta, track in zip(deltas, analysis_results):
        ...
        row["is_outlier"] = any(v["severity"] == "outlier" for v in row["violations"])
        out.append(row)
    return out


def _delta_check(
    *, metric: str, delta: float | None, tolerance: float, correctable: bool
) -> dict[str, Any]:
    if delta is None:
        return {
            "metric": metric,
...
def build_correction_plan(
    ...
    for cls in classifications:
        ...
        spectral_violations = [
            v
            for v in violations
            if v["metric"] in ("low_rms", "vocal_rms") and v["severity"] == "outlier"
        ]
        uncorrectable_outliers = [
            v
            for v in violations
            if v["metric"] in ("stl_95", "lra_floor") and v["severity"] == "outlier"
        ]
