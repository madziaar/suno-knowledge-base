...
_STEM_KEYWORDS = [
    ...
    (
        "woodwinds",
        ["woodwind", "flute", "clarinet", "oboe", "bassoon", "saxophone", "sax"],
    ),
    ("synth", ["synth"]),
...
def apply_highpass(data: Any, rate: int, cutoff: int = 30) -> Any:
    ...
    if cutoff <= 0 or cutoff >= nyquist:
        if cutoff > 0:
            logger.warning(
                "Highpass cutoff %d Hz out of range (0\u2013%.0f Hz), skipping",
                cutoff,
                nyquist,
            )
        return data
...
def _envelope_follower_python(
    abs_signal: Any, attack_coeff: float, release_coeff: float
) -> Any:
    ...
    def _repair_linear(channel: Any, indices: Any) -> Any:
        ...
        for idx in indices:
            ...
            while right < n - 1 and int(right) in click_set:
                right += 1
            if left != right:
                result[idx] = channel[left] + (channel[right] - channel[left]) * (
                    idx - left
                ) / (right - left)
        return result
...
def apply_sub_bass_exciter(
    data: Any, rate: int, amount: float = 0.0, freq: float = 80.0
) -> Any:
...
def remix_stems(
    stems_dict: dict[str, tuple[Any, int]], gains_dict: dict[str, float] | None = None
) -> tuple[Any, int]:
...
def _resolve_master_click_thresholds(
    genre: str | None,
...
def process_drums(
    ...
    if attack_db != 0 or sustain_db != 0:
        data = apply_transient_shaper(
            data, rate, attack_gain=attack_db, sustain_gain=sustain_db
        )
...
def process_bass(
    ...
    if exciter_amount > 0:
        ...
        data = apply_sub_bass_exciter(
            data, rate, amount=exciter_amount, freq=exciter_freq
        )
    ...
    for stem_name in STEM_NAMES:
        ...
        if not dry_run:
            ...
            settings = _get_stem_settings(
                stem_name, genre, analyzer_rec=stem_recs or None
            )
...
def mix_track_full(
    ...
    if not dry_run:
        ...
        high_tame_freq = settings.get("high_tame_freq", 7000)
        if high_tame_db != 0:
            data = apply_high_shelf(
                data, rate, freq=high_tame_freq, gain_db=high_tame_db
            )
        ...
        if comp_ratio > 1.0:
            data = gentle_compress(
                data, rate, threshold_db=comp_threshold, ratio=comp_ratio
            )
...
def _process_one_track(
    track_dir: Path | str, output_path: Path | str, genre: str | None, dry_run: bool
...
def _process_one_full_mix(
    wav_file: Path | str, output_path: Path | str, genre: str | None, dry_run: bool
...
Examples:
  ...
  python mix_tracks.py . --genre hip-hop --dry-run
    ...
    parser.add_argument(
        "--genre", "-g", type=str, default=None, help="Apply genre preset"
    )
    parser.add_argument(
        "--full-mix", action="store_true", help="Process full mix WAVs instead of stems"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        ...
        if workers == 1:
            for wav_file in wav_files:
                ...
                if result:
                    ...
                    print(
                        f"{name[:34]:<35} {result['pre_peak']:>9.4f} {post_peak:>9.4f}"
                    )
        else:
            ...
            with ProcessPoolExecutor(max_workers=workers) as executor:
                futures = {
                    executor.submit(
                        _process_one_track,
                        td,
                        output_dir / f"{td.name}.wav",  # type: ignore[arg-type]
                        args.genre,
                        args.dry_run,
                    ): i
                    for i, td in tasks
