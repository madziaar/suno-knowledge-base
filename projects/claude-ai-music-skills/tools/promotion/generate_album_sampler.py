...
def generate_album_sampler(
    ...
    try:
        ...
        for i, audio_file in enumerate(audio_files):
            ...
            title = (titles or {}).get(audio_file.stem) or get_track_title(
                audio_file.name
            )
        ...
        logger.info(
            "Concatenating %d clips with %ss crossfades...", len(clip_paths), crossfade
        )
        success = concatenate_with_crossfade(
            clip_paths, output_path, crossfade, clip_duration
        )
...
Examples:
    ...
    parser.add_argument(
        "tracks_dir", type=Path, help="Directory containing mastered tracks"
    )
    parser.add_argument(
        "--artwork",
        "-a",
    ...
    parser.add_argument(
        "-o", "--output", type=Path, help="Output path (default: album_sampler.mp4)"
    )
    parser.add_argument(
        "--clip-duration",
        type=int,
    ...
    parser.add_argument(
        "--artist", type=str, help="Artist name (read from config if not set)"
    )
    parser.add_argument(
        "--style",
        "-s",
        ...
        default="pulse",
        help="Waveform visualization style (default: pulse)",
    )
    parser.add_argument(
        "--color",
        type=str,
    ...
    if args.artwork:
        ...
        if not artwork:
            ...
            logger.error(
                "  Looked for: album.png, album.jpg, album-art.png, artwork.png, cover.png"
            )
    ...
    track_count = sum(
        1 for f in count_dir.iterdir() if f.suffix.lower() in audio_extensions
    )
    expected_duration = (
        track_count * args.clip_duration - (track_count - 1) * args.crossfade
    )
    ...
    if expected_duration > 140:
        ...
        logger.warning(
            "Consider reducing --clip-duration to %ds or less", int(140 / track_count)
        )
        print()
