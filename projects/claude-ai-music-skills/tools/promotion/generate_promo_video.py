...
def generate_waveform_video(
    ...
    logger.debug(
        "Wave color: %s, Text color: %s, Glow: %.1f", color2, effective_text_color, glow
    )
    ...
    artist_file = tempfile.NamedTemporaryFile(
        mode="w", suffix=".txt", delete=False
    )  # noqa: SIM115
...
Examples:
    ...
    parser.add_argument(
        "--batch", type=Path, help="Batch process all audio in directory"
    )
    parser.add_argument(
        "--batch-artwork",
        type=Path,
    ...
    parser.add_argument(
        "-o", "--output", type=Path, help="Output path (file or directory for batch)"
    )
    parser.add_argument(
        "--duration",
        "-d",
    ...
    parser.add_argument(
        ...
        default="bars",
        help="Waveform visualization style",
    )
    parser.add_argument(
        "--start", type=float, help="Start time in seconds (auto-detect if not set)"
    )
    parser.add_argument(
    ...
    if args.batch:
        ...
        if args.batch_artwork:
            ...
            if args.album:
                content_root = Path(
                    config.get("paths", {}).get("content_root", "")
                ).expanduser()
                if content_root.exists():
                    # Search for album in content directory
                    for genre_dir in (
                        content_root / "artists" / artist_name / "albums"
                    ).glob("*"):
                            ...
                            if not artwork:
                                for pattern in artwork_patterns:
                                    ...
                                    if candidate.exists():
                                        artwork = candidate
                                        logger.info(
                                            "Found artwork in content dir: %s", artwork
                                        )
                                        break
                            break
            ...
            if not artwork:
                ...
                logger.error(
                    "  Or use: /bitwize-music:import-art to copy artwork to audio folder"
                )
                sys.exit(1)
