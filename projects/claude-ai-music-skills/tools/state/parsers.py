...
def parse_frontmatter(text: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(result, dict):
            return {
                "_error": f"Frontmatter is not a mapping (got {type(result).__name__})"
            }
        return result
...
def parse_album_readme(path: Path) -> dict[str, Any]:
    ...
    result["title"] = fm.get("title", "").strip('"').strip("'") or _extract_heading(
        text
    )
    ...
    if isinstance(streaming_fm, dict):
        result["streaming_urls"] = {
            k: v for k, v in streaming_fm.items() if isinstance(v, str) and v.strip()
        }
    else:
        result["streaming_urls"] = {}
