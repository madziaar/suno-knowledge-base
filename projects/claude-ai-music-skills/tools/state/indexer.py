...
def build_config_section(config: dict[str, Any]) -> dict[str, Any]:
    ...
    overrides_dir = (
        str(resolve_path(overrides_raw))
        if overrides_raw
        else str(Path(content_root) / "overrides")
    )
    ...
    generation_section = {
        ...
        "require_suno_link_for_final": bool(
            gen_config.get("require_suno_link_for_final", True)
        ),
    ...
    return {
        ...
        "audio_root": str(
            resolve_path(paths.get("audio_root", content_root_raw + "/audio"))
        ),
        "documents_root": str(
            resolve_path(paths.get("documents_root", content_root_raw + "/documents"))
        ),
...
def build_state(
    config: dict[str, Any], plugin_root: Path | None = None
) -> dict[str, Any]:
...
def incremental_update(
    ...
    if config_section["config_mtime"] != old_config_mtime:
        ...
        path_fields_changed = config_section.get("content_root") != old_config.get(
            "content_root"
        ) or config_section.get("artist_name") != old_config.get("artist_name")
        ideas_path_changed = path_fields_changed or config.get("paths", {}).get(
            "ideas_file"
            ...
            if existing_album and existing_album.get("readme_mtime") == readme_mtime:
                ...
                if "_error" not in album_data:
                    ...
                    if existing_album and existing_album.get("tracks"):
                        ...
                        _update_tracks_incremental({"tracks": tracks}, album_dir)
                    else:
                        tracks = scan_tracks(album_dir)
...
def _update_tracks_incremental(album: dict[str, Any], album_dir: Path) -> None:
    ...
    album["tracks_completed"] = sum(
        1 for t in existing_tracks.values() if t.get("status") in completed_statuses
    )


def _acquire_lock_with_timeout(
    lock_fd: Any, timeout: int | float = LOCK_TIMEOUT_SECONDS
) -> None:
...
def write_state(state: dict[str, Any]) -> None:
    ...
    try:
        ...
        tmp_fd = tempfile.NamedTemporaryFile(  # noqa: SIM115
            mode="w", dir=CACHE_DIR, suffix=".tmp", prefix=".state_", delete=False
        )
...
def validate_state(state: dict[str, Any]) -> list[str]:
    ...
    if "plugin_version" in state:
        ...
        if pv is not None and not isinstance(pv, str):
            errors.append(
                f"plugin_version should be string or null, got {type(pv).__name__}"
            )
            ...
            if isinstance(tracks, dict):
                for track_slug, track in tracks.items():
                    ...
                    for key in ("path", "title", "status"):
                        if key not in track:
                            errors.append(
                                f"Track '{slug}/{track_slug}' missing '{key}'"
                            )
    else:
...
def cmd_rebuild(args: argparse.Namespace) -> int:
    ...
    track_count = sum(len(a.get("tracks", {})) for a in state["albums"].values())
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    if version != CURRENT_VERSION:
        logger.warning(
            "Version mismatch: state=%s, expected=%s", version, CURRENT_VERSION
        )
    else:
...
def cmd_session(args: argparse.Namespace) -> int:
    ...
    if args.clear:
        ...
        if args.add_action:
            ...
            if len(actions) >= 100:
                logger.error(
                    "Too many pending actions (max 100). Use --clear to reset."
                )
                return 1
...
def cmd_cleanup(args: argparse.Namespace) -> int:
    ...
    if not stale:
        logger.info(
            "All %d album paths exist on disk, nothing to clean up", len(albums)
        )
        return 0
...
def cmd_show(args: argparse.Namespace) -> int:
    ...
    for slug, album in albums.items():
        ...
        status_color = (
            Colors.GREEN
            if status == "Released"
            else (Colors.YELLOW if status == "In Progress" else Colors.NC)
        )
        print(
            f"  {slug} ({genre}) - {status_color}{status}{Colors.NC} [{completed}/{track_count} tracks]"
        )
    ...
    if model_counts:
        tier_parts = [
            f"{tier}: {count}" for tier, count in sorted(model_counts.items())
        ]
...
Examples:
    ...
    subparsers.add_parser(
        "update", help="Incremental update (only re-parse changed files)"
    )
    ...
    show_parser = subparsers.add_parser(
        "show", help="Pretty-print current state summary"
    )
    ...
    cleanup_parser = subparsers.add_parser(
        "cleanup", help="Remove albums from cache that no longer exist on disk"
    )
    cleanup_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be removed without changing state",
    )

    # session
    session_parser = subparsers.add_parser(
        "session", help="Update session context in state.json"
    )
    ...
    session_parser.add_argument(
        "--clear", action="store_true", help="Clear all session data before applying"
    )
