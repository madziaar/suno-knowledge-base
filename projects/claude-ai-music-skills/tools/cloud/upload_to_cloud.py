...
def get_s3_client(config: dict[str, Any]) -> Any:
    ...
    if provider == "r2":
        ...
        if not all([account_id, access_key, secret_key]):
            logger.error(
                "R2 credentials not configured in ~/.bitwize-music/config.yaml"
            )
            logger.error(
                "Required fields: cloud.r2.account_id, cloud.r2.access_key_id, cloud.r2.secret_access_key"
            )
            sys.exit(1)
        ...
        if not all([access_key, secret_key]):
            logger.error(
                "S3 credentials not configured in ~/.bitwize-music/config.yaml"
            )
            logger.error(
                "Required fields: cloud.s3.access_key_id, cloud.s3.secret_access_key"
            )
            sys.exit(1)
...
def find_album_path(
    config: dict[str, Any], album_name: str, audio_root_override: str | None = None
) -> Path:
    ...
    genre_matches = [
        m for m in genre_matches if m.is_dir() and _is_within(m, audio_root)
    ]
    ...
    if (
        os.sep in album_name
        or "/" in album_name
        or any(c in album_name for c in "*?[]")
    ):
        logger.error(
            "Album name '%s' contains invalid characters (path separators or glob patterns)",
            album_name,
        )
        sys.exit(1)
...
def upload_file(
    ...
    if dry_run:
        ...
        logger.info(
            "             Size: %s, Type: %s", format_size(file_size), content_type
        )
        return True
...
Examples:
    ...
    if not files:
        ...
        logger.error(
            "Generate videos with: /bitwize-music:promo-director %s", args.album
        )
        sys.exit(1)
    ...
    if public_read and not args.dry_run:
        ...
        if provider == "r2":
            ...
            print(
                f"  https://{bucket}.s3.{region}.amazonaws.com/{artist}/{args.album}/"
            )
