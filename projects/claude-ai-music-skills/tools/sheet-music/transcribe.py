...
def resolve_album_path(album_name: str) -> Path | None:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Album path resolves outside audio_root (possible path traversal): %s",
                album_path,
            )
            return None
...
def transcribe_track(
    anthemscore: str, wav_file: Path, output_dir: Path, args: argparse.Namespace
) -> bool:
...
Examples:
  ...
  %(prog)s sample-album --midi --dry-run   # Preview with MIDI
    ...
    parser.add_argument(
        "--pdf-only", action="store_true", help="Only generate PDF (skip MusicXML)"
    )
    parser.add_argument(
        "--xml-only", action="store_true", help="Only generate MusicXML (skip PDF)"
    )
    parser.add_argument("--midi", action="store_true", help="Also generate MIDI files")
    parser.add_argument("--treble", action="store_true", help="Treble clef only")
    parser.add_argument("--bass", action="store_true", help="Bass clef only")
    parser.add_argument(
        "--output", help="Output directory (default: sheet-music/ in source dir)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        ...
        if album_path:
            ...
            if config:
                try:
                    ...
                    logger.error(
                        "  2. Album path: %s/%s/%s", audio_root, artist, args.source
                    )
    ...
    print("=" * 40)
    print(
        f"{Colors.GREEN}Complete: {success_count}{Colors.NC} | {Colors.RED}Failed: {failed_count}{Colors.NC}"
    )
    print()
