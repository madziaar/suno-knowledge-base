...
def export_pdf(
    xml_path: Path, pdf_path: Path, musescore_path: str, dry_run: bool = False
) -> bool:
...
def prepare_xml(
    ...
    if match:
        ...
        content = content.replace(
            f"<work-title>{old_title}</work-title>", f"<work-title>{title}</work-title>"
        )
...
def _add_title_page_and_footer(
    pdf_path: Path,
    title: str,
    artist: str | None,
    ...
    try:
        ...
        page_size = songbook_mod.PAGE_SIZES.get(
            page_size_name, songbook_mod.PAGE_SIZES["letter"]
        )
        ...
        logger.warning(
            "  pypdf/reportlab not installed — skipping title page and footer for %s",
            pdf_path.name,
        )
    except Exception as e:
...
def resolve_source_dir(given_path: str | Path) -> tuple[Path, Path]:
    ...
    has_numbered = any(re.match(r"^\d+", f.stem) for f in given.glob("*.xml")) or any(
        re.match(r"^\d+", f.stem) for f in given.glob("*.musicxml")
    )
    if has_numbered:
        logger.info(
            "Using flat directory as source (backward compatibility): %s", given
        )
...
def prepare_singles(
    ...
    if source_manifest:
        ...
        xml_files = sorted(
            [f for f in source_path.glob("*.xml") if re.match(r"^\d+", f.stem)]
        )
        musicxml_files = sorted(
            [f for f in source_path.glob("*.musicxml") if re.match(r"^\d+", f.stem)]
        )
        xml_files.extend(musicxml_files)

        pdf_files_legacy = {
            f.stem: f for f in source_path.glob("*.pdf") if re.match(r"^\d+", f.stem)
        }
        mid_files_legacy = {
            f.stem: f for f in source_path.glob("*.mid") if re.match(r"^\d+", f.stem)
        }
    ...
    for track_num, source_slug, title in ordered_tracks:
        ...
        if xml_match:
            out_xml = prepare_xml(
                xml_match, singles_path, title, dry_run, output_name=singles_name
            )
            if out_xml:
            ...
            if xml_match and musescore and not xml_only:
                ...
                if not xml_only:
                    if not musescore and xml_match:
                        logger.warning(
                            "  No MuseScore available, copying source PDF (title may show track number)"
                        )
                    if not dry_run:
                        shutil.copy2(pdf_src, pdf_dst)
                        pdf_written = True
...
def main() -> None:
    ...
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Only show warnings and errors"
    )
    ...
    if not args.xml_only:
        ...
        if not musescore:
            logger.warning(
                "MuseScore not found — PDFs will be copied from source (titles may show track numbers)."
            )
