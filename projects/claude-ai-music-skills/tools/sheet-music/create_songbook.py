...
def create_toc_page(
    tracks: list[tuple[str, int]], page_size: tuple[float, float]
) -> Any:
...
def create_section_header(
    track_name: str, track_num: int, page_size: tuple[float, float]
) -> Any:
...
def create_songbook(
    ...
    if manifest:
        ...
        numbered = sorted(
            [f for f in source_path.glob("*.pdf") if re.match(r"^\d+", f.stem)]
        )
        if numbered:
            pdf_files = numbered
        else:
...
def main() -> None:
    ...
    parser.add_argument("source_dir", help="Directory containing sheet music PDFs")
    parser.add_argument(
        "--title", "-t", default="Songbook", help="Book title (default: Songbook)"
    )
    parser.add_argument(
        "--artist", "-a", help="Artist name (auto-detected from config if not provided)"
    )
    parser.add_argument(
        "--output", "-o", help="Output file path (default: [source_dir]/[title].pdf)"
    )
    parser.add_argument(
        "--page-size",
        "-p",
    ...
    parser.add_argument(
        "--cover", "-c", help="Path to cover image (auto-detected if not provided)"
    )
    parser.add_argument(
        "--website",
        "-w",
    ...
    if not section_headers and config:
        try:
            section_headers = config.get("sheet_music", {}).get(
