...
function Card({ className, ...props }: React.ComponentProps<"div">): React.JSX.Element {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-surface text-card-foreground flex flex-col gap-[var(--space-6)] rounded-xl border py-[var(--space-6)]",
        className
      )}
      {...props}
    />
...
function CardHeader({ className, ...props }: React.ComponentProps<"div">): React.JSX.Element {
  return (
    <div
      ...
      className={cn(
        "@container/card-header grid auto-rows-min grid-rows-[auto_auto] items-start gap-[var(--space-2)] px-[var(--space-6)] has-data-[slot=card-action]:grid-cols-[1fr_auto] [.border-b]:pb-[var(--space-6)]",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">): React.JSX.Element {
  return (
    <div
      ...
      className={cn("flex items-center px-[var(--space-6)] [.border-t]:pt-[var(--space-6)]", className)}
      {...props}
    />
  )
}
