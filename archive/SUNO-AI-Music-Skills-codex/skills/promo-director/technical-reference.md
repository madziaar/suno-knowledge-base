...
**Basic (everything):**
```
`promo-director` skill my-album
```

**Tracks only:**
```
`promo-director` skill my-album --tracks-only
```

**Sampler only:**
```
`promo-director` skill my-album --sampler-only
```

**Custom style:**
```
`promo-director` skill my-album --style neon
```

**Custom duration:**
```
`promo-director` skill my-album --duration 30
```
...
**Next Steps:**
1. [Optional] Generate promo videos: `promo-director` skill my-album
2. Begin release workflow: `release-director` skill my-album
```
...
**Optional:** Upload to cloud storage: `cloud-uploader` skill my-album

Ready for release workflow: `release-director` skill my-album
```
...
**default model** - This skill coordinates workflow and runs scripts. Creative output is in the videos themselves (generated by ffmpeg), not by the LLM.
