...
Check available ideas: `album-ideas` skill list
```
...
If you want to rename or re-scaffold, use `rename` skill on the
...
Set the genre first: `album-ideas` skill edit "No Genre"
```
...
Options:
1. Supply a different slug: `promote-idea` skill "Kleine Welt" kleine-welt-2
2. Resume the existing album: `resume` skill kleine-welt
```
...
```
`promote-idea` skill "Kleine Welt"
```
...
```
`promote-idea` skill "The Great Molasses Flood" molasses-1919
```
...
```
`promote-idea` skill "The Great Molasses Flood" documentary
```
...
2. ``new-album` skill <slug> <genre>` with a slug the user invented
...
2. **One-way operation** — Promotion creates files and updates state. There
   is no "unpromote"; use ``rename` skill` or manual cleanup if you
   need to redo.
