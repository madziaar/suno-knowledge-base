...
- `{plugin_root}` = the directory containing this CLAUDE.md file (= `this skill package root` in skills)
- `[artist]` = `artist.name`
...
1. **Verify setup** — Quick dependency check:
   ...
   ```
   - If MCP missing → **Stop immediately** and suggest: ``setup` skill mcp`
   - If config missing → suggest: ``configure` skill`
...
1.5. **Health check** — Use `health_check` MCP tool (checks venv + skill registration):
   ...
   - Venv `status: "no_venv"` → **stop** and suggest ``setup` skill`
...
2. **Load config** — Read `~/.bitwize-music/config.yaml`. If missing, tell user to run ``configure` skill`.
...
4.5. **Check for plugin upgrades** — Compare `plugin_version` in state.json vs `VERSION`:
...
5. _(Removed — run ``skill-model-updater` skill check` manually when new models are released)_
6. **Report from MCP state**:
   - Health warnings (from step 1.5 — omit if ok):
     ...
     - Skills stale: "⚠️ N skill(s) missing from Codex, N ghost — run: `refresh this Codex skill package`"
...
7. **Show contextual tips** based on state:
   - No albums → suggest ``tutorial` skill`
   - Ideas exist → suggest ``album-ideas` skill list`
   - In-progress albums → suggest ``resume` skill [album-name]`
   - Overrides loaded → note it; missing → suggest creating them (see `config/README.md` for override file reference)
   - Pending verifications → warn and suggest ``verify-sources` skill`
...
- **Album mentioned** → ``resume` skill`
- **"Make a new album"** → IMMEDIATELY use ``new-album` skill` BEFORE any discussion
- **"Turn idea into album" / "promote [idea]"** → ``promote-idea` skill "<idea title>"` (one-shot: creates album from a Pending idea, injects concept, updates status)
- **Writing lyrics** → apply ``lyric-writer` skill` expertise (auto-invokes suno-engineer)
- **Refining/polishing lyrics** → ``lyric-refiner` skill` (post-writing multi-pass refinement)
- **Planning album** → apply ``album-conceptualizer` skill` (7 planning phases required)
- **Suno prompts** → apply ``suno-engineer` skill` expertise (usually auto-invoked by lyric-writer; use directly only for re-prompting)
- **Research needed** → apply ``researcher` skill` standards
- **Polishing audio / fixing Suno artifacts** → apply ``mix-engineer` skill` expertise
- **Mastering audio** → polish first via ``mix-engineer` skill`, then apply ``mastering-engineer` skill` standards. Skip polish only if: (a) user says "master only", "skip polish", or "already polished"; or (b) polished audio already exists at `{audio_root}/artists/[artist]/albums/[genre]/[album]/polished/`. Applies equally to single-track and whole-album mastering.
- **Album art** → apply ``album-art-director` skill`
- **Writing promo copy** → apply ``promo-writer` skill` expertise
- **Releasing** → apply ``release-director` skill`

- **Checking for plagiarism** → ``plagiarism-checker` skill` (web search + LLM knowledge)
- **Checking voice/authenticity** → ``voice-checker` skill` (detect AI-sounding patterns)
- **Verifying sources** → ``verify-sources` skill` (human verification gate)
...
3. After adding sources → status: `❌ Pending` → human verifies via ``verify-sources` skill` → `✅ Verified (DATE)`
4. Block generation if verification incomplete — ``pre-generation-check` skill` enforces this
...
- `Sources Verified`: Human confirmed all sources via ``verify-sources` skill`
...
- ``verify-sources` skill` → when all tracks verified, advance album to `Sources Verified`
...
2. **Decide the fix path**:
   - **Style issue** (wrong genre, tempo, mood) → Revise Style Box via ``suno-engineer` skill`, then regenerate on Suno
   - **Lyrics issue** (wrong words, pronunciation) → Fix lyrics via ``lyric-writer` skill`, re-run ``pronunciation-specialist` skill`, then regenerate
...
Track files: zero-padded (`01-`, `02-`). Import with ``import-track` skill`, ``import-audio` skill`.
...
**Version files (must stay in sync)**: `VERSION` and `VERSION`
...
**Development workflow**: Feature branch off `develop` → Conventional Commits → ``test` skill all` → PR into `develop` → Release: merge `develop` → `main`. See [CONTRIBUTING.md](CONTRIBUTING.md) for details.
